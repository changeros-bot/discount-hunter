...
function loadOfficialVerificationMap() {
  const map = new Map();
  const modules = ["./v17-market-91-hubb-official-verification.js"];
  for (const modulePath of modules) {
    try {
      const mod = require(modulePath);
      for (const value of Object.values(mod || {})) {
        const result = typeof value === "function" ? value() : value;
        if (result?.symbol) map.set(String(result.symbol).toUpperCase(), result);
      }
    } catch {
...
function applyEvidenceOverlay(row, evidenceMap) {
  ...
  return {
    ...
    evidenceStatus: evidence.sourceStatus,
