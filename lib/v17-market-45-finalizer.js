...
function hasBlocker(row) {
  const raw = `${row?.status || ""} ${row?.risk || ""} ${row?.blocker || ""} ${row?.evidenceBlocker || ""} ${row?.financialBlocker || ""} ${row?.sourceBlocker || ""}`;
  const text = raw.replace(/NOT[_\s-]?BLOCKER|NOT[_\s-]?A[_\s-]?BLOCKER|RISK[_\s-]?NOT[_\s-]?BLOCKER|NON[_\s-]?BLOCKER|NOT[_\s-]?BLOCKING/gi, "");
...
function evidenceState(row) {
  ...
  if (/VERIFIED|EVIDENCE_PASS|SOURCE_VERIFIED|OFFICIAL_VERIFIED|official_q1_2026_verified|FORMAL_ONLY/i.test(text)) return "VERIFIED";
...
function permissionPaper(row) {
  return row?.permissions?.paperTrading === true;
}

function permissionFormal(row) {
  return row?.permissions?.formalObservation === true;
}

function paperEligible(row) {
  const score = scoreOf(row);
  return ((row?.quality === "PASSED_DRAFT" && score >= 15) || permissionPaper(row)) && isVerified(row) && !hasBlocker(row);
}

function formalEligible(row) {
  const score = scoreOf(row);
  return (((row?.quality === "PASSED_DRAFT" || row?.tier === "DEEP_REVIEW") && score >= 12) || permissionFormal(row)) && isVerified(row) && !hasBlocker(row);
}
...
export function finalizeMarket45Review(review) {
  ...
  for (const row of deduped) {
    ...
    if (paperEligible(row)) {
      finalPaper.push(mark(row, "紙上交易測試", "已完成批次 Evidence 決策，列入 7 天紙上交易測試；仍禁止真實交易。", { evidenceState: eState }));
      continue;
    }

    if (formalEligible(row)) {
      finalFormal.push(mark(row, "正式觀察", "已完成批次 Evidence 決策，列入正式觀察；仍不得進真實自動交易。", { evidenceState: eState }));
      continue;
    }
  ...
  return {
    ...
    finalRules: {
      ...
      paperTradingDays: 7,
      paperQualityGate: "Evidence = VERIFIED，且 permissions.paperTrading = true 或 PASSED_DRAFT + score >= 15，且沒有 Blocker",
      formalWatchGate: "Evidence = VERIFIED，且 permissions.formalObservation = true 或符合正式觀察分數門檻，且沒有 Blocker",
      pendingCandidateGate: "分數 / Quality 達標但 Evidence = PENDING，只能進待驗證候選",
      realAutoTradeAllowed: false,
      rule: "Market45 最終分類由批次 Evidence、分數、Quality Gate 與 Blocker 決定；禁止真實自動交易。",
    },
    finalSummary: Object.fromEntries(Object.entries(finalBuckets).map(([key, rows]) => [key, rows.length])),
    finalBuckets,
    completionText: completed
      ? `Market45 已完成門檻式最終篩選：紙上交易 ${finalBuckets["紙上交易測試"].length} 檔，正式觀察 ${finalBuckets["正式觀察"].length} 檔，待驗證候選 ${finalBuckets["待驗證候選"].length} 檔。`
      : "Market45 尚未達到資料完整條件。",
  };
}
