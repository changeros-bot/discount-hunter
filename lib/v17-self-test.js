...
export function runV17SelfTest() {
  ...
  tests.push(assert(btc?.discountModel === "btc_cycle_high_v1" && btc?.strategy === STRATEGIES.DCA_DISCOUNT, "btc_uses_dedicated_model", "BTC must use btc_cycle_high_v1 and DCA + Discount strategy.", { btc }));
  ...
  const d1Complete = buildV17Decisions({ assets: [btcAsset], markets: { BTC: { symbol: "BTC", price: 90000, cycleHigh: 100000 } }, events: [{ symbol: "BTC", price: 90000, amount: 5 }], now: "2026-06-29T00:00:00.000Z" });
  tests.push(assert(btcDecision(d1Complete)?.status === ACTION_STATUS.COMPLETE && btcDecision(d1Complete)?.tier === "D1" && d1Complete.actionCount === 0, "d1_complete_no_today_decision", "D1 completed must not stay in Today Decision.", { decision: btcDecision(d1Complete), actionCount: d1Complete.actionCount }));

  const d2AfterD1Complete = buildV17Decisions({ assets: [btcAsset], markets: { BTC: { symbol: "BTC", price: 80000, cycleHigh: 100000 } }, events: [{ symbol: "BTC", price: 90000, amount: 5 }], now: "2026-06-29T00:00:00.000Z" });
  tests.push(assert(btcDecision(d2AfterD1Complete)?.status === ACTION_STATUS.QUEUED && btcDecision(d2AfterD1Complete)?.tier === "D2" && d2AfterD1Complete.actionCount === 1, "d2_reenters_after_d1_complete", "D1 completed must not block D2 from entering Today Decision.", { decision: btcDecision(d2AfterD1Complete), actionCount: d2AfterD1Complete.actionCount }));
  ...
  const d1Skipped = buildV17Decisions({ assets: [btcAsset], markets: { BTC: { symbol: "BTC", price: 90000, cycleHigh: 100000 } }, events: [{ symbol: "BTC", type: "skip_layer", layer: 1, source: "ui_skip_layer" }], now: "2026-06-29T00:00:00.000Z" });
  tests.push(assert(btcDecision(d1Skipped)?.status === ACTION_STATUS.SKIPPED && d1Skipped.actionCount === 0, "skip_current_layer_leaves_queue", "Skipped current layer must leave action queue.", { decision: btcDecision(d1Skipped), actionCount: d1Skipped.actionCount }));

  const d2AfterD1Skipped = buildV17Decisions({ assets: [btcAsset], markets: { BTC: { symbol: "BTC", price: 80000, cycleHigh: 100000 } }, events: [{ symbol: "BTC", type: "skip_layer", layer: 1, source: "ui_skip_layer" }], now: "2026-06-29T00:00:00.000Z" });
  tests.push(assert(btcDecision(d2AfterD1Skipped)?.status === ACTION_STATUS.QUEUED && btcDecision(d2AfterD1Skipped)?.tier === "D2" && d2AfterD1Skipped.actionCount === 1, "deeper_layer_reenters_after_skip", "D1 skipped must not block D2 from entering action queue.", { decision: btcDecision(d2AfterD1Skipped), actionCount: d2AfterD1Skipped.actionCount }));
