...
function isRecordedCompleteEvent(event, eventLayer) {
  const type = String(event?.type || event?.eventType || "").toLowerCase();
  const status = String(event?.status || "").toLowerCase();
  const source = String(event?.source || "").toLowerCase();
  const id = String(event?.id || "").toLowerCase();
  const intent = String(event?.completionIntent || "").toLowerCase();
  return eventLayer > 0
    && type === "manual_buy"
    && status === "filled"
    && source === "v17_ui"
    && (intent === "user_confirmed_current_layer_complete" || id.includes("-complete-"));
}

export function summarizeAssetEvents({ asset, layer, events }) {
  const currentRequiredAmount = Number(asset.amounts?.[layer - 1] || 0);
  let totalAmount = 0;
  let priceMissing = false;
  let failed = false;
  let skipped = false;
  let recordedCompletion = false;
  let recordedRequiredAmount = null;
  const matched = [];
  const mismatch = [];
  const skippedEvents = [];
  ...
  for (const event of events) {
    ...
    const expectedAmount = toNumber(event?.expectedAmount ?? event?.requiredAmount, null);
    ...
    if (eventLayer === layer) {
      totalAmount += amount;
      if (isRecordedCompleteEvent(event, eventLayer)) {
        const snapshotAmount = expectedAmount && expectedAmount > 0 ? expectedAmount : amount;
        recordedRequiredAmount = recordedRequiredAmount === null
          ? snapshotAmount
          : Math.max(recordedRequiredAmount, snapshotAmount);
        if (snapshotAmount > 0 && amount + AMOUNT_TOLERANCE_USD >= snapshotAmount) recordedCompletion = true;
      }
      matched.push({ ...event, layer: eventLayer, amount, expectedAmount, priceMissing: !hasPrice });
    } else if (eventLayer > 0) {
      const priorRequired = toNumber(event?.expectedAmount ?? event?.requiredAmount, Number(asset.amounts?.[eventLayer - 1] || 0));
      const isPriorLayerDone = eventLayer < layer && amount <= Number(priorRequired || 0) + AMOUNT_TOLERANCE_USD;
  ...
  const requiredAmount = recordedCompletion && recordedRequiredAmount > 0
    ? recordedRequiredAmount
    : currentRequiredAmount;
  const complete = recordedCompletion || (requiredAmount > 0 && totalAmount + AMOUNT_TOLERANCE_USD >= requiredAmount);
  ...
  return {
    ...
    currentRequiredAmount,
    recordedRequiredAmount,
    completionBasis: recordedCompletion ? "recorded_rule_snapshot" : "current_registry_rule",
    ...
    priceMissing,
    amountLow: partial,
    amountHigh: !recordedCompletion && requiredAmount > 0 && totalAmount > requiredAmount + AMOUNT_TOLERANCE_USD,
    matched,
    mismatch,
    skippedEvents
  };
...
export function buildStateDecision({ asset, pricedAsset, context, events, previousState, now }) {
  ...
  if (!isQualified) {
    ...
    reason = summary.completionBasis === "recorded_rule_snapshot"
      ? "Layer completed under the rule snapshot recorded when the user confirmed completion."
      : "Required amount completed by matched events.";
  ...
  const previousDetectedAmount = toNumber(
    previousState?.detectedAmount ?? previousState?.eventSummary?.totalAmount,
    0
  ) || 0;
  const detectedAmountChanged = Math.abs(previousDetectedAmount - summary.totalAmount) > AMOUNT_TOLERANCE_USD;
  const shouldNotify = status !== ACTION_STATUS.SKIPPED && (
    !previousState ||
    previousState.status !== status ||
    previousState.tier !== deepest.tier ||
    detectedAmountChanged
  );
  ...
  return {
    ...
    rule: deepest.rule,
    amount,
    detectedAmount: summary.totalAmount,
    price: pricedAsset.price ?? null,
    high: pricedAsset.high ?? pricedAsset.cycleHigh ?? null,
    discount: pricedAsset.discount ?? null,
