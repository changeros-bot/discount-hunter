...
export function buildStateDecision({ asset, pricedAsset, context, events, previousState, now }) {
  ...
  } else if (summary.priceMissing && summary.totalAmount > 0) {
  ...
  const shouldNotify = !previousState || previousState.status !== status || previousState.tier !== deepest.tier || previousState.price !== pricedAsset.price;
