...
function memoryKeyFromPath(filePath) {
  return filePath === ALERTS_PATH ? "alerts" : "ledger";
}
...
export function canReopenTier({ ledger, symbol, tier, now = new Date().toISOString() }) {
  ...
  return lastBuy.leftBuyZone === true && Boolean(lastBuy.leftBuyZoneAt) && hoursBetween(lastBuy.time, now) > 24;
}
...
export function getNextProgress(asset) {
  ...
  if (!Number.isFinite(currentDepth) || !ruleDepths.length) {
    return { stageText: "資料未就緒", fromText: "0U", toText: "0U", progress: 0, displayProgress: 0, triggered: false };
  }
...
export async function readAlerts() {
  return readStoreJson({ key: ALERTS_KEY, filePath: ALERTS_PATH, fallback: {} });
}

export async function writeAlerts(alerts) {
  return writeStoreJson({ key: ALERTS_KEY, filePath: ALERTS_PATH, value: alerts || {} });
}
