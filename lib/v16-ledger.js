...
function looseSymbolKey(symbol) {
  return String(symbol || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
}

export function normalizeSymbol(symbol) {
  const raw = String(symbol || "").trim();
  const rawKey = looseSymbolKey(raw);
  const matched = SYMBOLS.find((s) => looseSymbolKey(s) === rawKey);
...
export function normalizeLedger(saved = {}) {
  ...
  for (const [rawSymbol, rawRows] of Object.entries(saved || {})) {
    let symbol;
    try { symbol = normalizeSymbol(rawSymbol); } catch { continue; }
    base[symbol] = { ...base[symbol], ...(rawRows || {}) };
  }
...
export async function writeStoreJson({ key, filePath, value }) {
  ...
  if (requiresDurableKv()) {
    throw new Error("missing_required_upstash_kv");
  }
  const memoryKey = memoryKeyFromPath(filePath);
...
function getLedgerRows(ledger, symbol) {
  let normalizedSymbol;
  try { normalizedSymbol = normalizeSymbol(symbol); } catch { return {}; }
  if (ledger?.[normalizedSymbol]) return ledger[normalizedSymbol];
  const target = looseSymbolKey(normalizedSymbol);
  const matchedKey = Object.keys(ledger || {}).find((key) => looseSymbolKey(key) === target);
...
export function getLastBuy(ledger, symbol, tier) {
  ...
  const rows = getLedgerRows(ledger, symbol)?.[normalizedTier] || [];
...
export async function appendBuy({ symbol, tier, amount, price, note, time }) {
  ...
  const ledger = await readLedger();
  const buyTime = time || new Date().toISOString();

  if (DIP_TIERS.includes(normalizedTier) && !canReopenTier({ ledger, symbol: normalizedSymbol, tier: normalizedTier, now: buyTime })) {
    const existing = getLastBuy(ledger, normalizedSymbol, normalizedTier);
    return {
  ...
  const row = buildManualBuy({ symbol: normalizedSymbol, tier: normalizedTier, amount: numericAmount, price, note, time: buyTime });
  ledger[normalizedSymbol][normalizedTier].push(row);
  const writeResult = await writeLedger(ledger);
  return { symbol: normalizedSymbol, tier: normalizedTier, row, ledger: ledger[normalizedSymbol], storage: writeResult.store, duplicate: false };
}
