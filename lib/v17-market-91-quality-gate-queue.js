...
const QUALITY_GATE_RULES = {
  version: "v17-market-91-quality-gate-queue-v14-original-completion-formal-added",
...
function buildQueueCandidate(row) {
  ...
  const priorityMap = {
    ...
    LLY: { priority: 13, mainRisk: "GLP-1 durability, manufacturing capacity, pipeline concentration, pricing/policy risk, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_PHARMA_PLATFORM_BUT_GLP1_CONCENTRATION_AND_VALUATION_MUST_BE_VERIFIED" },
    COST: { priority: 14, mainRisk: "Membership renewal, comparable sales, membership fee income, gross margin, wage/cost pressure, international growth, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_MEMBERSHIP_RETAIL_BUT_VALUATION_AND_MARGIN_MUST_BE_VERIFIED" },
    SPOT: { priority: 15, mainRisk: "Gross margin durability, label economics, ad monetization, user growth, podcast ROI, and valuation must be verified.", preliminaryGateBias: "AUDIO_PLATFORM_CANDIDATE_BUT_LABEL_ECONOMICS_MARGIN_AND_VALUATION_MUST_BE_VERIFIED" },
    TMUS: { priority: 16, mainRisk: "Subscriber growth, ARPU, churn, spectrum/capex, leverage, FCF, and valuation must be verified.", preliminaryGateBias: "WIRELESS_FCF_CANDIDATE_BUT_CAPEX_LEVERAGE_AND_GROWTH_MUST_BE_VERIFIED" },
    NFLX: { priority: 17, mainRisk: "Subscriber growth, ad-tier monetization, content spend, margin durability, competition, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_STREAMING_PLATFORM_BUT_CONTENT_SPEND_AND_VALUATION_MUST_BE_VERIFIED" },
    NOW: { priority: 18, mainRisk: "Armis acquisition, margin / FCF compression, and software growth durability must be verified.", preliminaryGateBias: "LIKELY_STRONG_BUT_MARGIN_AND_ACQUISITION_MUST_BE_VERIFIED" },
    MA: { priority: 19, mainRisk: "Consumer spending cycle, cross-border volume, regulation/interchange pressure, network competition, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_PAYMENT_NETWORK_BUT_REGULATION_VOLUME_AND_VALUATION_MUST_BE_VERIFIED" },
    V: { priority: 20, mainRisk: "Payments volume, cross-border recovery, regulation/interchange litigation, competition, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_PAYMENT_NETWORK_BUT_REGULATION_AND_VOLUME_MUST_BE_VERIFIED" },
    GEV: { priority: 21, mainRisk: "Grid demand, gas power cycle, backlog quality, margin expansion, project execution, services mix, and valuation must be verified.", preliminaryGateBias: "AI_POWER_INFRASTRUCTURE_CANDIDATE_BUT_BACKLOG_MARGIN_AND_EXECUTION_MUST_BE_VERIFIED" },
    BKNG: { priority: 22, mainRisk: "Travel demand cycle, room nights growth, take rate, marketing efficiency, regulation, and valuation must be verified.", preliminaryGateBias: "HIGH_CASHFLOW_TRAVEL_PLATFORM_BUT_CYCLE_REGULATION_AND_VALUATION_MUST_BE_VERIFIED" },
    ISRG: { priority: 23, mainRisk: "Procedure growth, system placements, recurring instrument revenue, margin durability, competition, hospital CapEx cycle, and valuation must be verified.", preliminaryGateBias: "HIGH_QUALITY_ROBOTIC_SURGERY_PLATFORM_BUT_PROCEDURE_GROWTH_AND_VALUATION_MUST_BE_VERIFIED" },
    ROP: { priority: 24, mainRisk: "Organic growth, acquisition discipline, FCF quality, leverage, software mix, and valuation must be verified.", preliminaryGateBias: "QUALITY_VERTICAL_SOFTWARE_COMPOUNDER_BUT_MA_AND_VALUATION_MUST_BE_VERIFIED" },
    MU: { priority: 25, mainRisk: "HBM ramp, pricing, margin durability, CapEx, supply growth, and customer qualification must be verified.", preliminaryGateBias: "PROMISING_AI_MEMORY_CORE_BUT_HBM_MARGIN_AND_CAPEX_MUST_BE_VERIFIED" },
    ACN: { priority: 26, mainRisk: "Consulting demand, bookings, discretionary IT spending, AI transformation revenue, margin, and valuation must be verified.", preliminaryGateBias: "IT_SERVICES_AI_TRANSFORMATION_CANDIDATE_BUT_DEMAND_AND_MARGIN_MUST_BE_VERIFIED" },
    VRT: { priority: 27, mainRisk: "Backlog quality, margin durability, delivery execution, working capital, and valuation sensitivity must be verified.", preliminaryGateBias: "PROMISING_BUT_EXECUTION_AND_BACKLOG_QUALITY_MUST_BE_VERIFIED" },
    INTU: { priority: 28, mainRisk: "AI disruption in tax/accounting, TurboTax and QuickBooks growth durability, SMB cycle, margin durability, and valuation must be verified.", preliminaryGateBias: "QUALITY_VERTICAL_SOFTWARE_BUT_AI_TAX_DISRUPTION_AND_VALUATION_MUST_BE_VERIFIED" },
  };
...
function getQualityGateQueue() {
  ...
  const seen = new Set();
  const eligibleRaw = report.groups.formalObservationCandidateOnly || [];
  const eligible = eligibleRaw
    .filter((row) => { if (seen.has(row.symbol)) return false; seen.add(row.symbol); return true; })
