const rows = [
  ...
  ["NFLX",82.0,15.0,7.5,7.5,"QUALITY_GATE_DRAFT_PASS_NO_PERMISSION","High-quality streaming platform draft pass. Subscriber growth, ad-tier monetization, content spend, margin durability, competition, and valuation require verification."],
  ["BKNG",81.0,15.0,7.5,7.5,"QUALITY_GATE_DRAFT_PASS_NO_PERMISSION","High-cash-flow online travel platform draft pass. Room nights, gross bookings, marketing efficiency, travel cycle, regulation, and valuation require verification."],
  ["ISRG",81.0,15.0,7.5,7.5,"QUALITY_GATE_DRAFT_PASS_NO_PERMISSION","High-quality robotic surgery platform draft pass. Procedure growth, system placements, recurring instrument revenue, hospital CapEx cycle, competition, margin durability, and valuation require verification."],
...
const riskMap = {
  NVDA: ["export controls", "inventory/supply-chain commitments", "customer concentration", "ASIC/TPU competition"], MSFT: ["AI CapEx", "Azure margin", "Copilot monetization", "OpenAI dependency", "valuation"], AVGO: ["VMware integration", "debt paydown", "customer concentration", "valuation vs FCF"], TSM: ["geopolitical risk", "overseas fab cost", "CapEx intensity", "AI customer concentration"], MA: ["cross-border volume", "regulation/interchange pressure", "consumer spending cycle", "valuation"], V: ["payments volume", "cross-border volume", "regulation/litigation", "competition", "valuation"], GOOGL: ["search AI disruption", "AI CapEx", "regulatory remedies", "cloud margin"], META: ["AI CapEx", "FCF durability", "Reality Labs losses", "regulatory/litigation risk"], ANET: ["cloud titan concentration", "AI Ethernet cycle", "gross margin", "inventory"], AMZN: ["AWS growth/margin", "AI/data-center CapEx", "retail margin", "FCF"], NFLX: ["subscriber growth", "ad-tier monetization", "content spend", "margin durability", "competition", "valuation"], BKNG: ["room nights", "gross travel bookings", "take rate", "marketing efficiency", "regulation", "valuation"], ISRG: ["procedure growth", "system placements", "instrument/service revenue", "hospital CapEx cycle", "competition", "valuation"], NOW: ["Armis integration", "subscription growth", "margin/FCF", "AI workflow disruption"], MU: ["HBM ramp", "pricing", "margin", "CapEx", "customer qualification"], INTU: ["TurboTax growth", "QuickBooks growth", "AI tax/accounting disruption", "SMB cycle", "valuation"], VRT: ["backlog conversion", "working capital", "margin", "liquid cooling demand", "valuation"],
};

function makeRow([symbol, fairScore, totalScore, objectiveScore, qualitativeScore, status, verdict]) {
  const risks = riskMap[symbol] || [];
  return { symbol, source: "Market 91 Quality Gate Queue", fairScore, status, confidence: "draft_quality_gate_requires_official_evidence", totalScore, maxScore: 18, objectiveScore, qualitativeScore, permission: { buy: false, dca: false, semiAuto: false, whitelist: false }, verdict,
    objectiveChecks: ["Revenue trend", "Free cash flow quality", "Margin quality", "Balance sheet / leverage", "CapEx / capital discipline"].map((name) => ({ name, score: Math.min(2, Math.max(1, objectiveScore / 5)), max: 2, status: "草稿", reason: "Requires official evidence before permission review." })),
    qualitativeChecks: ["Industry position", "Moat / switching cost / scale", "Management / capital allocation", "Thesis durability"].map((name) => ({ name, score: Math.min(2, Math.max(1, qualitativeScore / 4)), max: 2, status: "草稿", reason: "Requires official evidence before permission review." })),
...
function getQualityGateDrafts() {
  return { version: "v17-market-91-quality-gate-drafts-v9-healthcare-expanded", policy: "draft_quality_gate_only_no_trading_permission", scoring: { objectiveMax: 10, qualitativeMax: 8, totalMax: 18, pass: 14, strong: 16, tradingReview: 18 }, summary: { total: QUALITY_GATE_DRAFTS.length, draftPass: QUALITY_GATE_DRAFTS.filter((x) => x.totalScore >= 14).length, strongDraft: QUALITY_GATE_DRAFTS.filter((x) => x.totalScore >= 16).length, tradingPermission: 0, reviewOrder: QUALITY_GATE_DRAFTS.map((x) => x.symbol) }, rows: QUALITY_GATE_DRAFTS };
}
