...
function classifyExcluded(row) {
  const text = `${row.tier || ""} ${row.status || ""} ${row.risk || ""} ${row.reason || ""} ${row.rule || ""} ${row.blocker || ""}`;
  ...
  if (/OBJECTIVE_RISK_BLOCKED|INVERSE_LEVERAGED|CASH_TOOL|BOND|CLO|TBILL|ETF|COMMODITY_ROLL|SECTOR_ETF|COUNTRY_ETF/i.test(text)) {
    return {
  ...
  if (/SECOND_REVIEW|RESERVE|二審|只能人工研究|週期|能源|鋼鐵/i.test(text)) {
    return {
      auditCategory: "有訊號但分數 / 風險不足",
...
export function getMarket91AuditTrail() {
  ...
  return {
    ok: true,
    version: "v17-market-91-audit-trail-v3-complete-46-review",
