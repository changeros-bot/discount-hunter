...
function isActiveOpenTrade(trade, asset, today, testDays) {
  ...
  const sourceMatch = !asset.paperSource || !trade.sourceType || String(trade.sourceType) === String(asset.paperSource);
  const groupMatch = !asset.paperGroup || !trade.group || String(trade.group) === String(asset.paperGroup);
  const ageDays = daysBetween(trade.dateKey || trade.createdAt, today);
  return sourceMatch && groupMatch && ageDays >= 0 && ageDays < Number(testDays || 7);
}

function inferPaperGroup(trade, meta) {
  if (trade?.group) return trade.group;
  if (meta?.paperGroup) return meta.paperGroup;
  if (/既有10檔|既有V17十檔/i.test(String(trade?.source || ""))) return "既有V17十檔";
  return "未分類";
}

function inferPaperSourceType(trade, meta) {
  if (trade?.sourceType) return trade.sourceType;
  if (meta?.paperSource) return meta.paperSource;
  if (/既有10檔|既有V17十檔/i.test(String(trade?.source || ""))) return "existing_ten";
  return null;
}
...
export async function runAutoPaperTrading({ markets = {}, force = false } = {}) {
  ...
  for (const asset of eligible) {
    ...
    if (activeDuplicate) {
      skipped.push({ symbol: asset.symbol, group: asset.paperGroup, reason: "已有 7 天內 OPEN 紙上測試；防重複建倉", existingId: activeDuplicate.id, forceIgnored: Boolean(force) });
      continue;
    }
  ...
  return {
    ...
    skipped,
    storage,
    duplicateGuard: "active open paper trade with same symbol/source/group blocks new lots even when force=true",
...
export async function getPaperSummary({ markets = {}, persistMetrics = false } = {}) {
  ...
  const playbookMap = new Map(dedupeAssets([...existingAssets, ...market45Assets]).map((asset) => [paperKey(asset.symbol), asset]));
  ...
  const positions = openTrades.map((trade) => {
    ...
    const group = inferPaperGroup(trade, meta);
    const sourceType = inferPaperSourceType(trade, meta);
  ...
  return {
    ...
    market45PaperCandidates: market45?.finalBuckets?.["紙上交易測試"] || [],
    summary: {
      ...
      market45CandidateCount: market45?.finalBuckets?.["紙上交易測試"]?.length || 0,
      groups,
...
export async function readMarket45Review() {
  ...
  return {
    ...
    note: missingCount > 0
      ...
      : "45 檔已完成收斂；Market 45 需通過 finalBuckets 紙上交易測試門檻後，才可進入紙上交易。",
  };
}
