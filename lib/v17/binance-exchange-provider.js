...
const DEFAULT_BINANCE_REST_URL = BINANCE_PROXY_REST_URL;
...
function getBinanceRestUrl() {
  const configured = String(process.env.BINANCE_REST_BASE_URL || "").trim().replace(/\/$/, "");
  if (!configured) return DEFAULT_BINANCE_REST_URL;

  const lower = configured.toLowerCase();
  const blockedOrServerlessProxy =
    lower.includes("api.binance.com") ||
    lower.includes("vercel.app") ||
    lower.includes("railway.app") ||
    lower.includes("render.com") ||
    lower.includes("netlify.app");

  if (blockedOrServerlessProxy) return BINANCE_PROXY_REST_URL;
  return configured;
}
...
function normalizeBtcPosition({ account, trades, marketPrice, tradeMeta = {} }) {
  ...
  const cost = hasTrades
    ...
    : { quantity: balance.quantity, totalCost: 0, averageBuyPrice: 0, buyCount: 0, sellCount: 0, firstBuyTime: null, lastBuyTime: null, lastSellTime: null, tradeCount: 0 };
  ...
  return {
    ...
    costBasisSource: hasCostBasis ? "binance_myTrades_weighted_average" : "missing_binance_myTrades_cost_basis",
    costBasisMissing: !hasCostBasis,
    costBasisWarning: hasCostBasis ? null : "BTC balance is real from Binance account, but myTrades cost basis is unavailable or empty.",
    averageBuyPriceSource: hasCostBasis ? "binance_myTrades_weighted_average" : "missing_binance_myTrades_cost_basis",
