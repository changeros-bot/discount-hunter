...
function getFairScoreReport() {
  ...
  const third = getThirdBatch();
  const fourth = getFourthBatch();
  const fifth = getFifthBatch();
  const sixth = getSixthBatch();
  const seventh = getSeventhBatch();
  const eighth = getEighthBatch();
  const ninth = getNinthBatch();
  const tenth = getTenthBatch();
  const eleventh = getEleventhBatch();
  const rows = [
    ...flattenBatch("first", first),
    ...flattenBatch("second", second),
    ...flattenBatch("third", third),
    ...flattenBatch("fourth", fourth),
    ...flattenBatch("fifth", fifth),
    ...flattenBatch("sixth", sixth),
    ...flattenBatch("seventh", seventh),
    ...flattenBatch("eighth", eighth),
    ...flattenBatch("ninth", ninth),
    ...flattenBatch("tenth", tenth),
    ...flattenBatch("eleventh", eleventh),
  ...
  return {
    version: "v17-market-91-fair-score-report-v10-first-to-eleventh-batch",
    policy: "100_point_screen_observation_only_18_point_quality_gate_required_for_permissions",
    safetyBoundary: [
      "First to eleventh batch scores are not a buy list.",
    ...
    includedBatches: [
      ...
      { name: "third", version: third.version, count: third.summary.total },
      { name: "fourth", version: fourth.version, count: fourth.summary.total },
      { name: "fifth", version: fifth.version, count: fifth.summary.total },
      { name: "sixth", version: sixth.version, count: sixth.summary.total },
      { name: "seventh", version: seventh.version, count: seventh.summary.total },
      { name: "eighth", version: eighth.version, count: eighth.summary.total },
      { name: "ninth", version: ninth.version, count: ninth.summary.total },
      { name: "tenth", version: tenth.version, count: tenth.summary.total },
      { name: "eleventh", version: eleventh.version, count: eleventh.summary.total },
    ],
    summary: {
      total: rows.length,
      ...
      completedFromMarket91: rows.length,
      remainingFromMarket91: 91 - rows.length,
    },
    rows,
    groups,
  };
