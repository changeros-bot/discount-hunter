...
export async function findExecutionByDraftId(draftId) {
  const logs = await readExecutionLog();
  return logs.find((entry) => entry.draftId === draftId) || null;
}
...
export async function appendExecutionLog({ draft, status = "SIMULATED", error = null }) {
  ...
  const logs = await readExecutionLog();
  const existing = logs.find((entry) => entry.draftId === draft.id);
  if (existing) return { entry: existing, storage: { store: "unchanged" }, duplicate: true };
  ...
  return { entry, storage, duplicate: false };
}
