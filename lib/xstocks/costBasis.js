// DCA Discount Hunter V15.9 - Cost Basis Engine
// Reconstructs official buy-side holdings by matching stablecoin OUT + xStock IN in the same txHash.
// TRANSFER_IN / airdrop / unknown mint records are preserved in buyRecords debug, but excluded from official holdings and PnL.
...
function looksRawInteger(value, decimals) {
  ...
  // Raw ERC20 values for 18-decimal tokens are usually long integers.
  // Short values like "2000" from indexers are often already formatted display amounts.
  ...
  for (const [hash, txs] of groups.entries()) {
    ...
    for (const xtx of xstockInflows) {
      ...
      records.push({
        ...
        ...(type === "BUY" ? {} : { warning: `No stablecoin outflow found for hash ${hash} - excluded from official holdings and PnL` }),
      });
    }
    ...
    if (xstockOutflows.length > 0 && stableInUsd > 0) {
      for (const xtx of xstockOutflows) {
        records.push({
          ...
          officialHolding: false,
          warning: "Sell detected; V15 official holdings use buy-side cost basis only. FIFO realized PnL deferred to V16.",
        });
      }
    }
  }
...
function calculateHoldings(records) {
  ...
  for (const r of records) {
    ...
    // Only real BUY records enter official holdings. TRANSFER_IN remains visible in debug only.
    if (!symbol || r.type !== "BUY") continue;
    ...
    if (!existing) {
      ...
      if (!existing.firstBuyTimestamp || Number(r.timestamp) < Number(existing.firstBuyTimestamp)) {
        existing.firstBuyTimestamp = r.timestamp;
      }
      if (!existing.lastBuyTimestamp || Number(r.timestamp) > Number(existing.lastBuyTimestamp)) {
