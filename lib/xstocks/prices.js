...
function pickContractAddress(item) {
...
async function getBinanceDynamic(item) {
  ...
  const contractAddress = pickContractAddress(item);
  if (!contractAddress) throw new Error(`missing EVM contractAddress for ${getSymbol(item) || "unknown"}`);
