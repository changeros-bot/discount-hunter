// DCA Discount Hunter V15.6 - Live Binance xStocks price fetchers
// Note: Binance public metadata may expose internal token IDs such as CT_501 for price dynamic API.
// For price fetching we allow those IDs. Only chain balance logic should require real 0x EVM contracts.
...
const headers = {
  ...
  "user-agent": "discount-hunter/15.6",
};
...
function collectEvmAddresses(value, path = "root", out = []) {
  if (!value) return out;

  if (typeof value === "string") {
    const clean = value.trim();
    if (isEvmAddress(clean)) out.push({ address: clean, path });
    return out;
  }

  if (Array.isArray(value)) {
    value.forEach((item, index) => collectEvmAddresses(item, `${path}[${index}]`, out));
    return out;
  }

  if (typeof value === "object") {
    for (const [key, child] of Object.entries(value)) {
      collectEvmAddresses(child, `${path}.${key}`, out);
    }
  }
...
function scoreAddressCandidate(candidate) {
  ...
  let score = 0;
  if (path.includes("contract")) score += 100;
  if (path.includes("token")) score += 40;
  if (path.includes("address")) score += 30;
  if (path.includes("bsc") || path.includes("bnb") || path.includes("chain")) score += 20;
  if (path.includes("owner") || path.includes("wallet") || path.includes("issuer")) score -= 100;
  return score;
}

function pickPriceContractId(item) {
  const directCandidates = [
    item?.contractAddress,
    item?.tokenContractAddress,
    item?.tokenAddress,
    item?.bscContractAddress,
    item?.bscTokenAddress,
    item?.address,
  ...
  const candidates = collectEvmAddresses(item).sort((a, b) => scoreAddressCandidate(b) - scoreAddressCandidate(a));
...
async function getBinanceDynamic(item) {
  ...
  const contractAddress = pickPriceContractId(item);
  if (!contractAddress) throw new Error(`missing Binance price contract/id for ${getSymbol(item) || "unknown"}`);
...
async function fetchTokenMetadata(symbols) {
  ...
  return requested.map((symbol) => {
    ...
    if (!item) return null;
    const allEvmAddresses = collectEvmAddresses(item).map((x) => ({ address: x.address, path: x.path, score: scoreAddressCandidate(x) }));
    const contractAddress = pickPriceContractId(item);
    return {
      symbol,
