...
// Raw ledger is not chain transfer-history cost.
// Verified tx hash sources are chain receipt/log verified and override raw ledger rows for the same symbol.
...
function aggregateRawLedgerRows(rows = []) {
  ...
  const rawItems = [...map.values()].map((item) => ({
    ...item,
  ...
  return applyVerifiedTxCostSources(rawItems);
}
