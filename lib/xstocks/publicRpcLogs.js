...
// Uses standard eth_getLogs with block chunking and normalizes to the existing transfer shape.
...
async function getLogsInChunks(filterBase, fromBlock, toBlock) {
  const logs = [];
  const chunkSize = Number(process.env.PUBLIC_BSC_RPC_LOG_CHUNK_SIZE || process.env.BSC_RPC_LOG_CHUNK_SIZE || 20000);
...
async function fetchPublicRpcTransfers(walletAddress) {
  ...
  const [inLogs, outLogs] = await Promise.all([
    getLogsInChunks(inboundFilter, fromBlock, toBlock),
    getLogsInChunks(outboundFilter, fromBlock, toBlock),
  ]);
