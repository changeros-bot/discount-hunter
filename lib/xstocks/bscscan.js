// DCA Discount Hunter - BSC BEP20 transfer fallback
// Current stable fallback: Etherscan API V2 with chainid=56 first, then native BscScan.
...
function getEtherscanV2KeyCandidates() {
  // Etherscan V2 is the official multi-chain API. Try ETHERSCAN_API_KEY first.
  // Some deployments only have BSCSCAN_API_KEY set, so try it as a fallback key candidate.
  return uniq([getEtherscanApiKey(), getBscScanApiKey()]);
}
...
function normalizeBscScanTransfer(t) {
  return {
    ...
    bscscanRaw: { tokenSymbol: t.tokenSymbol, tokenName: t.tokenName, value: t.value, tokenDecimal: t.tokenDecimal },
  };
}
...
async function fetchPagedTransfers({ baseUrl, apiKey, walletAddress, useChainId = false, providerName }) {
  ...
  const startblock = String(process.env.XSTOCKS_TRANSFER_START_BLOCK || process.env.BSCSCAN_START_BLOCK || 0);
  const endblock = String(process.env.XSTOCKS_TRANSFER_END_BLOCK || process.env.BSCSCAN_END_BLOCK || 999999999);
  ...
  while (page <= maxPages) {
    ...
    let data;
    try { data = JSON.parse(text); } catch { throw new Error(`${providerName} returned non-json: ${text.slice(0, 180)}`); }
...
async function fetchNativeBscScanTransfers(walletAddress) {
  return fetchPagedTransfers({ baseUrl: BSCSCAN_NATIVE_URL, apiKey: getBscScanApiKey(), walletAddress, useChainId: false, providerName: "BscScan native" });
}
async function fetchEtherscanV2BscTransfersWithKey(walletAddress, apiKey, label = "Etherscan V2 chainid=56") {
  return fetchPagedTransfers({ baseUrl: ETHERSCAN_V2_URL, apiKey, walletAddress, useChainId: true, providerName: label });
}
async function fetchEtherscanV2BscTransfers(walletAddress) {
  const errors = [];
  for (const [index, key] of getEtherscanV2KeyCandidates().entries()) {
    try {
      const label = index === 0 && getEtherscanApiKey() ? "Etherscan V2 chainid=56" : "Etherscan V2 chainid=56 fallback key";
      const rows = await fetchEtherscanV2BscTransfersWithKey(walletAddress, key, label);
      if (rows.length > 0) return rows;
      errors.push(`${label} returned 0 transfers`);
    } catch (error) {
      errors.push(error.message);
    }
  }
  throw new Error(errors.join(" | ") || "Etherscan V2 API key not found");
}
async function fetchBep20Transfers(walletAddress) {
  const errors = [];

  // Prefer Etherscan V2. It is the current official multi-chain API and uses chainid=56 for BSC.
  if (getEtherscanV2KeyCandidates().length) {
    try {
    ...
    errors.push("ETHERSCAN_API_KEY / BSCSCAN_API_KEY missing for Etherscan V2");
  }
