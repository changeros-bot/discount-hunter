// DCA Discount Hunter V15.29 - BSC RPC balanceOf live wallet scanner
...
// If RPC balanceOf returns zero, BscScan tokenbalance is used as a fallback guard.

const { fetchTokenMetadata } = require("./prices");
const { getKnownContracts } = require("./contracts");
const { fetchBscScanTokenBalance } = require("./bscscan-balance");
...
function aggregateHoldingsBySymbol(contractHoldings) {
  ...
  for (const holding of contractHoldings || []) {
    ...
    existing.details.push({
      ...
      balanceSource: holding.balanceSource || null,
      callStatus: holding.callStatus || null,
      errorMessage: holding.errorMessage || null,
    });
...
async function getBalanceWithExplorerFallback(contractAddress, walletAddress) {
  const rpcRawBalance = await getBalanceOf(contractAddress, walletAddress);
  if (rpcRawBalance > 0n) {
    return { rawBalance: rpcRawBalance, balanceSource: "rpc_balanceOf", fallbackRawBalance: null, fallbackError: null };
  }

  try {
    const fallbackRaw = await fetchBscScanTokenBalance(walletAddress, contractAddress);
    const fallbackRawBalance = decimalStringToBigInt(fallbackRaw);
    if (fallbackRawBalance > 0n) {
      return { rawBalance: fallbackRawBalance, balanceSource: "bscscan_tokenbalance_fallback", fallbackRawBalance: fallbackRaw, fallbackError: null };
    }
    return { rawBalance: rpcRawBalance, balanceSource: "rpc_balanceOf_zero_bscscan_zero", fallbackRawBalance: fallbackRaw, fallbackError: null };
  } catch (error) {
    return { rawBalance: rpcRawBalance, balanceSource: "rpc_balanceOf_zero_bscscan_failed", fallbackRawBalance: null, fallbackError: error.message || "BscScan fallback failed" };
  }
}
...
async function fetchWalletBalancesViaRpc(walletAddress, symbols, contractHints = []) {
  ...
  await Promise.all(tokenMetadata.map(async (token) => {
    if (!isEvmAddress(token.contractAddress)) {
      const errorMessage = `invalid contractAddress ${token.contractAddress || "null"}`;
      errors.push(`${token.symbol}: ${errorMessage}`);
      contractHoldings.push({
        ...
        decimals: token.decimals || 18,
        isZeroBalance: false,
        callStatus: "invalid_contract",
        errorMessage,
        balanceSource: "invalid_contract",
      });
      return;
    }
    ...
    try {
      ...
      const balanceResult = await getBalanceWithExplorerFallback(token.contractAddress, cleanWalletAddress);
      const rawBalance = balanceResult.rawBalance;
      const quantity = formatUnits(rawBalance, decimals);
      const effectiveSource = balanceResult.balanceSource === "bscscan_tokenbalance_fallback"
        ? `${token.source || "bsc_rpc_balanceOf"}+bscscan_tokenbalance_fallback`
        : (token.source || "bsc_rpc_balanceOf");

      if (balanceResult.fallbackError) {
        errors.push(`${token.symbol}: ${balanceResult.fallbackError}`);
      }
      ...
      contractHoldings.push({
        ...
        callStatus: "ok",
        errorMessage: balanceResult.fallbackError || null,
        balanceSource: balanceResult.balanceSource,
        fallbackRawBalance: balanceResult.fallbackRawBalance,
      });
    } catch (error) {
      const errorMessage = error.message || "unknown RPC balanceOf error";
      errors.push(`${token.symbol}: ${errorMessage}`);
      contractHoldings.push({
        symbol: token.symbol,
        quantity: null,
        ...
        decimals: token.decimals || 18,
        isZeroBalance: false,
        callStatus: "rpc_error",
        errorMessage,
        balanceSource: "rpc_error",
      });
    }
  }));
