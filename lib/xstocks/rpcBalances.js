// DCA Discount Hunter V15.30 - BSC RPC balanceOf live wallet scanner
// Reads token balances directly from BNB Chain RPC.
// Priority: verified contract whitelist + wallet transfer contract hints, then Binance metadata.
// Important: verified contracts must not suppress wallet transfer hints.
// Rationale: xStocks contracts can change or wrapper addresses can differ by wallet route.
...
// Debug output distinguishes real zero balances from RPC / contract errors.
// Critical: eth_call result "0x" is EMPTY_RETURN / UNKNOWN, not zero balance.
...
function aggregateHoldingsBySymbol(contractHoldings) {
  ...
  for (const holding of contractHoldings || []) {
    ...
    existing.details.push({
      ...
      debugCallData: holding.debugCallData || null,
      debugCallDataLength: holding.debugCallDataLength || null,
      debugWalletAddressUsed: holding.debugWalletAddressUsed || null,
    });
...
function hexToBigIntStrict(hex) {
  if (!hex) throw new Error("BALANCEOF_EMPTY_RESULT");
  if (hex === "0x") throw new Error("BALANCEOF_EMPTY_RETURN");
  return BigInt(hex);
}
...
async function getTokenDecimals(contractAddress, fallbackDecimals = 18) {
  try {
    ...
    if (!result || result === "0x") return fallbackDecimals;
...
async function getBalanceOf(contractAddress, walletAddress) {
  ...
  return hexToBigIntStrict(result);
}
...
async function buildTokenList(symbols, contractHints = []) {
  ...
  // Do not suppress wallet transfer hints when a verified symbol exists.
  // A verified address can be stale or can represent a wrapper with zero balance,
  // while the actual wallet holding comes from the transfer-history contract address.
  const hintTokens = normalizeContractHints(contractHints)
    .filter((token) => requested.includes(upper(token.symbol)));

  const symbolsCoveredByVerifiedOrHints = new Set([
    ...verified.map((token) => upper(token.symbol)),
    ...hintTokens.map((token) => upper(token.symbol)),
  ]);

  const metadataTargets = requested.filter((symbol) => !symbolsCoveredByVerifiedOrHints.has(symbol));
  ...
  await Promise.all(tokenMetadata.map(async (token) => {
    if (!isEvmAddress(token.contractAddress)) {
      ...
      contractHoldings.push({
        ...
        errorMessage,
        debugCallData: null,
        debugCallDataLength: null,
        debugWalletAddressUsed: cleanWalletAddress,
      });
      return;
    }

    let debugCallData = null;
    let debugCallDataLength = null;

    try {
      debugCallData = `0x70a08231${padAddress(cleanWalletAddress)}`;
      debugCallDataLength = debugCallData.length;
      ...
      const callStatus = errorMessage.includes("BALANCEOF_EMPTY_RETURN")
        ? "balanceof_empty_return"
        : errorMessage.includes("BALANCEOF_EMPTY_RESULT")
          ? "balanceof_empty_result"
          : "rpc_error";
      errors.push(`${token.symbol}: ${errorMessage}`);
      contractHoldings.push({
        symbol: token.symbol,
