// DCA Discount Hunter V15.3 - BSC RPC balanceOf fallback
// Reads token balances directly from BNB Chain RPC.
// Priority: verified BscScan 0x contract mapping. Binance metadata contractAddress may be CT_501 internal IDs and must not be trusted for balanceOf().
...
async function buildTokenList(symbols) {
  const requested = (symbols || []).map(upper).filter(Boolean);
  const verified = getKnownContracts(requested);
  const verifiedSymbols = new Set(verified.map((t) => upper(t.symbol)));

  let metadata = [];
  const missingSymbols = requested.filter((symbol) => !verifiedSymbols.has(symbol));
...
async function fetchWalletBalancesViaRpc(walletAddress, symbols) {
  ...
  const tokenMetadata = await buildTokenList(symbols);
