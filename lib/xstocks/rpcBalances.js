// DCA Discount Hunter V15.16 - BSC RPC balanceOf live wallet scanner
// Reads token balances directly from BNB Chain RPC.
// Priority: wallet transfer contract hints > Binance metadata > optional static fallback.
// Every live balance is first read per contract, then aggregated by symbol while keeping contract details.
...
async function buildTokenList(symbols, contractHints = []) {
  ...
  const metadataTargets = requested.filter((symbol) => !hintedSymbols.has(symbol));
  const metadata = await fetchMetadataTokens(metadataTargets);
  const metadataSymbols = new Set(metadata.map((t) => upper(t.symbol)));

  const staticFallbackEnabled = String(process.env.ENABLE_STATIC_XSTOCK_CONTRACTS || "").toLowerCase() === "true";
  const staticTargets = staticFallbackEnabled ? requested.filter((symbol) => !hintedSymbols.has(symbol) && !metadataSymbols.has(symbol)) : [];
  const verified = staticFallbackEnabled ? getKnownContracts(staticTargets) : [];

  return dedupeTokens([...hintTokens, ...metadata, ...verified]);
}
...
async function fetchWalletBalancesViaRpc(walletAddress, symbols, contractHints = []) {
  ...
  const holdings = aggregateHoldingsBySymbol(contractHoldings);
