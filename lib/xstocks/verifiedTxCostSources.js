...
// Most entries are backed by /api/v17/tx-source-audit:
// stablecoin OUT from wallet + xStock IN to wallet in the same tx hash.
// AMDON is backed by BscScan transaction state + Binance position screenshots until the transfer parser supports its route.
...
const VERIFIED_TX_COST_SOURCES = {
  AMDON: {
    ...
    totalCost: 5,
    buyCount: 1,
    costBasisSource: "verified_bscscan_state_receipt",
    verifiedRule: "BscScan tx state touches AMDON contract + Binance position confirms AMDon holding",
...
function applyVerifiedTxCostSources(items = []) {
  ...
  for (const source of Object.values(VERIFIED_TX_COST_SOURCES)) {
    ...
    const costBasisSource = source.costBasisSource || "verified_tx_hash_receipt";
    map.set(symbol, {
      ...existing,
      symbol,
      ...
      verifiedRule: source.verifiedRule || "stablecoin OUT + xStock IN in same tx hash = BUY",
      recoveredSources: [...new Set([...(existing.recoveredSources || []), costBasisSource])],
