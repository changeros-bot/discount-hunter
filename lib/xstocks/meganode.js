...
function getMegaNodeApiKey() {
  return process.env.MEGANODE_API_KEY || process.env.NODEREAL_API_KEY || "";
}

function getMegaNodeEndpoint() {
  const custom = norm(process.env.MEGANODE_ENDPOINT || process.env.NODEREAL_ENDPOINT);
...
async function megaNodeRpc(method, params) {
  ...
  if (!endpoint) throw new Error("MEGANODE_API_KEY not found");
