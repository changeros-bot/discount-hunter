// DCA Discount Hunter V15.5 - MegaNode / NodeReal transfer fetcher
// Stable mode: use standard BSC JSON-RPC eth_getLogs for ERC20 Transfer events.
// Provider strategy: NodeReal/MegaNode first, then public BSC RPC fallback.
...
function getRpcEndpoints() {
  const customPublic = firstEnv(["BSC_RPC_URL", "NEXT_PUBLIC_BSC_RPC_URL"]);
  const configured = [getMegaNodeEndpoint(), customPublic, ...PUBLIC_BSC_RPC_URLS].filter(Boolean);
  return [...new Set(configured)];
}

function hasMegaNodeConfig() {
  return getRpcEndpoints().length > 0;
}
...
async function rpcAt(endpoint, method, params) {
  ...
  let json;
  try { json = JSON.parse(text); } catch { throw new Error(`RPC returned non-json: ${text.slice(0, 160)}`); }
...
async function rpc(method, params) {
  ...
  for (const endpoint of getRpcEndpoints()) {
    try {
      return await rpcAt(endpoint, method, params);
    } catch (error) {
      errors.push(`${endpoint}: ${error.message}`);
    }
  }
  throw new Error(`BSC RPC all endpoints failed: ${errors.join(" | ")}`);
}

async function getBlockNumber(endpoint) {
  const hex = endpoint ? await rpcAt(endpoint, "eth_blockNumber", []) : await rpc("eth_blockNumber", []);
...
async function getLogsInChunksAt(endpoint, filterBase, fromBlock, toBlock) {
  const logs = [];
  ...
  while (start <= end) {
    ...
    const rows = await rpcAt(endpoint, "eth_getLogs", [{ ...filterBase, fromBlock: toHex(start), toBlock: toHex(stop) }]);
...
async function fetchTokenTransfersAt(endpoint, walletAddress, tokenMeta, currentBlock) {
  const walletTopic = padTopicAddress(walletAddress);
  const lookback = Number(process.env.XSTOCKS_TRANSFER_LOOKBACK_BLOCKS || 5000000);
  const fromBlock = Number(process.env.XSTOCKS_TRANSFER_START_BLOCK || process.env.NODEREAL_START_BLOCK || Math.max(0, currentBlock - lookback));
  ...
  const [inLogs, outLogs] = await Promise.all([
    getLogsInChunksAt(endpoint, inboundFilter, fromBlock, currentBlock),
    getLogsInChunksAt(endpoint, outboundFilter, fromBlock, currentBlock),
  ]);
  return [...inLogs, ...outLogs].map((log) => logToTransfer(log, tokenMeta));
}

async function fetchAllTransfersAt(endpoint, walletAddress) {
  const cleanWallet = lower(walletAddress);
  const currentBlock = await getBlockNumber(endpoint);
  ...
  for (const token of tokens) {
    const rows = await fetchTokenTransfersAt(endpoint, cleanWallet, token, currentBlock);
    groups.push(rows);
  }
  const seen = new Set();
  return groups.flat()
...
async function fetchMegaNodeTransfers(walletAddress) {
  ...
  for (const endpoint of getRpcEndpoints()) {
    try {
      const rows = await fetchAllTransfersAt(endpoint, walletAddress);
      if (rows.length > 0) {
        console.log(`xStocks transfer logs selected endpoint: ${endpoint}, rows=${rows.length}`);
        return rows;
      }
      errors.push(`${endpoint}: 0 transfer logs`);
      best = rows;
    } catch (error) {
      errors.push(`${endpoint}: ${error.message}`);
    }
  }
  console.warn(`xStocks transfer logs returned zero from all RPC endpoints: ${errors.join(" | ")}`);
  return best;
}
