...
export function hasHolding({ asset, ledger = {}, walletMap = new Map() }) {
  return Boolean(walletHoldingFor(asset, walletMap)) || ledgerDoneTiers(ledger, asset?.symbol).length > 0;
}
...
export function classifyAsset({ asset, ledger = {}, walletMap = new Map(), decisionMap = new Map() }) {
  ...
  const holding = hasHolding({ asset, ledger, walletMap });
...
export function classifyUniverse({ assets = [], ledger = {}, holdings = [], decisions = [] }) {
  ...
  const rows = (assets || []).map((asset) => classifyAsset({ asset, ledger, walletMap, decisionMap }));
