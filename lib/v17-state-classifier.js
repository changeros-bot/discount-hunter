...
export function isPrimaryHoldingAsset(asset) {
  return asset?.assetType === "crypto";
}

export function resolveSection({ asset, level, decision, holding }) {
  if (level === 0) return V17_SECTIONS.WATCH;
  if (level > 0 && isPrimaryHoldingAsset(asset)) return V17_SECTIONS.HOLDING;
...
export function classifyAsset({ asset, ledger = {}, walletMap = new Map(), actionMap = new Map(), stateMap = new Map(), skippedMap = new Map(), completeMap = new Map() }) {
  ...
  const holding = hasHolding({ asset, ledger, walletMap, completeTiers: doneTiers }) || skippedTiers.includes(tier) || (isPrimaryHoldingAsset(asset) && level > 0);
  const section = resolveSection({ asset, level, decision, holding });
...
export function classifyUniverse({ assets = [], ledger = {}, holdings = [], decisions = [], states = [] }) {
  ...
  return {
    ...
    rules: {
      ...
      watch: "D0 only",
      decision: "D1-D4 active uncompleted/unskipped tier has priority over non-primary holdings",
      holding: "D1-D5 and holding/ledger exists; primary assets like BTC render here as their main card"
    }
  };
}
