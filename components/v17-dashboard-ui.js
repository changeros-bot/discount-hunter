...
function highLabel(row) { return row?.referenceMode === "cycle_high" || row?.discountModel === "btc_cycle_high_v1" || row?.assetType === "crypto" ? "週期高點" : "高點"; }
function drawdownLabel(row) { return row?.referenceMode === "cycle_high" || row?.discountModel === "btc_cycle_high_v1" || row?.assetType === "crypto" ? "距週期高點降幅" : "距52週高點降幅"; }
...
function dcaGroup(row) {
  const key = symbolKey(row?.symbol);
  const allowed = ["QQQON", "QQQ", "QQQM", "NVDAON", "TSMON", "AVGOON", "GOOGLON", "NVDA", "TSM", "AVGO", "GOOGL", "BTC"];
  return allowed.includes(key) ? "可定期定額" : "不可定期定額";
}

function qualityStatus(row) {
  const key = symbolKey(row?.symbol);
  if (key === "BTC") return { tone: "green", status: "通過", gate: "採用加密貨幣週期模型", action: "小額定期投入可｜深度回撤加碼可" };
  if (["QQQON", "QQQ", "QQQM"].includes(key)) return { tone: "green", status: "通過", gate: "採用指數型資產模型", action: "機械式定期定額可｜逢低加碼可" };
  if (["NVDAON", "TSMON", "AVGOON", "GOOGLON", "NVDA", "TSM", "AVGO", "GOOGL"].includes(key)) return { tone: "green", status: "通過", gate: "每季重新檢查公司品質", action: "定期定額可｜品質惡化時暫停" };
  if (["AMDON", "MRVLON", "AMD", "MRVL"].includes(key)) return { tone: "yellow", status: "通過", gate: "只採折價與品質雙重確認", action: "定期定額禁止｜只做逢低分批" };
  if (["RKLBON", "RKLB", "SPCXON", "SPCX"].includes(key)) return { tone: "red", status: "高風險", gate: "小額創投型部位，先限制最大損失", action: "定期定額禁止｜不得因下跌自動加碼" };
  return { tone: "yellow", status: "資料待確認", gate: "尚未完成分類", action: "定期定額禁止｜逢低待確認" };
}

function strategyPolicy(row) {
  const key = symbolKey(row?.symbol);
  if (["QQQON", "QQQ", "QQQM"].includes(key)) return { tone: "green", title: "可定期定額｜純定期定額", dca: "固定日期、固定金額，長期持續投入。", dip: "大幅下跌時可額外加碼，但不取代原本定期定額。", note: "最適合機械式長期累積。" };
  if (["NVDAON", "TSMON", "AVGOON", "GOOGLON", "NVDA", "TSM", "AVGO", "GOOGL"].includes(key)) return { tone: "green", title: "可定期定額｜混合策略", dca: "可按月投入，但每季必須重新檢查公司品質。", dip: "品質未惡化且價格進入折價層級時，可額外加碼。", note: "不是無條件一直買；投資邏輯惡化時必須暫停。" };
  if (key === "BTC") return { tone: "green", title: "可定期定額｜週期混合策略", dca: "可保留小額固定投入。", dip: "主要資金依週期高點回撤分層加碼，並受總部位上限約束。", note: "不使用公司財務指標，獨立採用週期與鏈上資料。" };
  if (["AMDON", "MRVLON", "AMD", "MRVL"].includes(key)) return { tone: "yellow", title: "不可定期定額｜只做逢低買進", dca: "不固定每月投入，時間到了不是買入理由。", dip: "價格進入折價層級且品質檢查通過後才分批買入。", note: "不得因價格一直跌就無限攤平。" };
  if (["RKLBON", "RKLB"].includes(key)) return { tone: "red", title: "不可定期定額｜小額創投型部位", dca: "先設定總投入上限，單筆只用小額資金，不按月固定買。", dip: "只有價格大幅折價、公司進展正常、投資邏輯仍成立時才可追加。", note: "下跌若來自發射失敗、進度嚴重延後、現金快速消耗或大量增發，立即停止加碼。" };
  if (["SPCXON", "SPCX"].includes(key)) return { tone: "red", title: "不可定期定額｜小額創投型部位", dca: "先設定比 RKLB 更低的總投入上限，單筆只用極小額資金。", dip: "只有母資產進展正常，且代幣映射、流動性、價差與發行結構都正常時才可追加。", note: "代幣與母資產偏離、流動性惡化、價差過大或發行結構異常時，直接停止買入。" };
  return { tone: "yellow", title: "不可定期定額｜未分類", dca: "未完成分類前禁止固定投入。", dip: "完成資料與品質確認後再決定。", note: "不得自動執行。" };
}
...
function cardGroupStyle(row) {
  const allowed = dcaGroup(row) === "可定期定額";
  return allowed
    ? { background: "radial-gradient(circle at 0% 0%, rgba(34,197,94,.22), transparent 38%), linear-gradient(135deg, rgba(6,78,59,.34), rgba(5,11,24,.82))", border: "1px solid rgba(34,197,94,.42)", glow: "rgba(34,197,94,.16)", dot: "#35f59a", badge: "#bbf7d0", badgeBg: "rgba(34,197,94,.16)", badgeBorder: "rgba(34,197,94,.35)" }
    : { background: "radial-gradient(circle at 0% 0%, rgba(248,113,113,.20), transparent 38%), linear-gradient(135deg, rgba(127,29,29,.28), rgba(5,11,24,.84))", border: "1px solid rgba(248,113,113,.42)", glow: "rgba(248,113,113,.14)", dot: "#fb7185", badge: "#fecaca", badgeBg: "rgba(248,113,113,.14)", badgeBorder: "rgba(248,113,113,.35)" };
}
...
function StrategyPolicyCard({ row }) {
  ...
  return <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
    <div style={{ padding: 10, borderRadius: 14, background: qt.bg, border: `1px solid ${qt.border}`, color: qt.color, fontWeight: 900, fontSize: 12, lineHeight: 1.5 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><span style={{ fontSize: 13, fontWeight: 1000 }}>品質狀態</span><span>{q.status}</span></div>
      <div style={{ marginTop: 5, color: "#e2e8f0" }}>{q.gate}</div>
      <div style={{ color: "#cbd5e1" }}>{q.action}</div>
    </div>
    <div style={{ padding: 10, borderRadius: 14, background: t.bg, border: `1px solid ${t.border}`, color: t.color, fontWeight: 900, fontSize: 12, lineHeight: 1.55 }}>
      <div style={{ fontSize: 13, fontWeight: 1000 }}>{p.title}</div>
      <div style={{ marginTop: 6 }}>{p.dca}</div>
      <div style={{ marginTop: 3 }}>{p.dip}</div>
      <div style={{ marginTop: 6, color: "#e2e8f0" }}>{p.note}</div>
    </div>
  </div>;
}
...
export function AssetCard({ row, children }) { const tone = TIER_TONE[row.tier] || TIER_TONE.D0; const group = cardGroupStyle(row); return <article style={{ position: "relative", overflow: "hidden", padding: 14, borderRadius: 22, background: group.background, border: group.border, color: "#f8fafc", boxShadow: `0 18px 40px rgba(0,0,0,.35), 0 0 28px ${group.glow}, inset 0 1px 0 rgba(255,255,255,.05)`, backdropFilter: "blur(18px)" }}><style jsx>{`.asset-card-children > div:first-child > span:last-child { display: none !important; }`}</style><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 10 }}><div style={{ display: "flex", gap: 8, alignItems: "center", minWidth: 0 }}><span style={{ flex: "0 0 auto", width: 15, height: 15, borderRadius: 999, background: group.dot, boxShadow: `0 0 16px ${group.dot}` }} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 21, fontWeight: 1000, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{row.symbol}</div><div style={{ color: "#94a3b8", fontWeight: 850, fontSize: 12 }}>{row.name || "--"}</div></div></div><div style={{ display: "grid", justifyItems: "end", gap: 5 }}><span style={{ padding: "5px 8px", borderRadius: 999, color: group.badge, background: group.badgeBg, border: `1px solid ${group.badgeBorder}`, fontSize: 11, fontWeight: 1000 }}>{dcaGroup(row)}</span><strong style={{ padding: "6px 10px", borderRadius: 999, background: tone.bg, border: `1px solid ${tone.border}`, color: tone.color, fontSize: 13, boxShadow: `0 0 18px ${tone.border}` }}>{row.tier}</strong></div></div><CardMetrics row={row} /><div className="asset-card-children" style={{ marginTop: 10 }}>{children}</div><StrategyPolicyCard row={row} /><LayerRules row={row} rules={row.rules || []} amounts={row.amounts || []} activeTier={row.tier} /></article>; }
...
export function PageShell({ loading, updatedAt, error, children }) { const liveColor = loading ? "#ffc857" : "#35f59a"; return <main style={{ minHeight: "100vh", padding: 12, background: "radial-gradient(circle at 12% 0%, rgba(49,231,255,.16), transparent 28%), radial-gradient(circle at 90% 8%, rgba(53,245,154,.10), transparent 24%), linear-gradient(180deg, #050b18, #020617)", color: "#f8fafc", fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" }}><header style={{ position: "relative", overflow: "hidden", padding: "18px 14px 14px", textAlign: "left", borderRadius: 28, background: "linear-gradient(135deg, rgba(11,19,36,.92), rgba(5,11,24,.76))", border: "1px solid rgba(49,231,255,.26)", boxShadow: "0 0 46px rgba(49,231,255,.10), inset 0 1px 0 rgba(255,255,255,.07)", backdropFilter: "blur(20px)" }}><div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}><a href="/semi-auto-drafts" style={{ padding: "6px 10px", borderRadius: 999, color: "#bbf7d0", background: "rgba(34,197,94,.10)", border: "1px solid rgba(34,197,94,.30)", fontSize: 11, fontWeight: 1000, letterSpacing: 1.1, textDecoration: "none" }}>半自動草稿</a><a href="/v17-quality" style={{ padding: "6px 10px", borderRadius: 999, color: "#fde68a", background: "rgba(245,158,11,.10)", border: "1px solid rgba(245,158,11,.30)", fontSize: 11, fontWeight: 1000, letterSpacing: 1.1, textDecoration: "none" }}>品質稽核</a></div><div style={{ padding: "6px 10px", borderRadius: 999, color: CYAN, background: "rgba(49,231,255,.08)", border: "1px solid rgba(49,231,255,.30)", fontSize: 11, fontWeight: 1000, letterSpacing: 1.7 }}>V17-M</div></div><h1 style={{ position: "relative", margin: "22px 0 20px", fontSize: "clamp(48px, 15vw, 82px)", lineHeight: .98, fontWeight: 1000, letterSpacing: -2.2, background: "linear-gradient(180deg, #f8fdff 0%, #31e7ff 52%, #35f59a 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textShadow: "0 0 36px rgba(49,231,255,.30)" }}>美股定期定額<br />折價追蹤</h1><div style={{ position: "relative", display: "flex", justifyContent: "flex-end", margin: "0 0 14px" }}><div style={{ padding: "8px 12px", borderRadius: 16, color: "#a5f3fc", background: "rgba(49,231,255,.08)", border: "1px solid rgba(49,231,255,.24)", fontSize: 11, fontWeight: 950, letterSpacing: 1.4 }}>BINANCE XSTOCKS</div></div><div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "10px 14px", borderRadius: 18, background: "rgba(2,6,23,.54)", border: "1px solid rgba(49,231,255,.18)" }}><div style={{ display: "flex", alignItems: "center", gap: 7, color: liveColor, fontWeight: 1000, fontSize: 12, letterSpacing: .8 }}><span style={{ width: 9, height: 9, borderRadius: 999, background: liveColor, boxShadow: `0 0 16px ${liveColor}` }} />{loading ? "同步中" : "即時"}</div><div style={{ color: "#cbd5e1", fontWeight: 900, fontSize: 12, letterSpacing: .4 }}>{timeText(updatedAt)}</div></div>{error && <div style={{ position: "relative", marginTop: 10, padding: 10, borderRadius: 14, color: "#fecaca", background: "rgba(127,29,29,.35)", fontWeight: 900 }}>{error}</div>}</header>{children}</main>; }
