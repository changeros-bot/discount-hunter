...
function progress(row) {
  const rules = arr(row?.rules).map(abs);
...
function Status({ row }) {
  const skipped = arr(row?.skippedTiers).includes(row?.tier);
  const done = arr(row?.ledgerDoneTiers).join(" / ");
  const text = row?.decision ? `待處理：${row?.tier || "--"}` : skipped ? `已略過：${row?.tier || "--"}` : row?.tier === "D0" ? "等待進入 D1" : `已完成：${done || row?.tier || "--"}`;
...
export function TierProgress({ row }) {
  ...
  return <div style={{ marginTop: 10, padding: 10, borderRadius: 16, background: "rgba(3,9,20,.38)", border: "1px solid rgba(49,231,255,.14)" }}>
    ...
    <div style={{ position: "relative", marginTop: 8, height: 8, borderRadius: 999, background: "rgba(49,231,255,.12)" }}><div style={{ width: `${p.pct}%`, height: "100%", borderRadius: 999, background: "linear-gradient(90deg,#35f59a,#31e7ff,#ffc857)" }} /></div>
...
function LayerRules({ rules = [], amounts = [], activeTier }) {
  const rs = arr(rules);
  return <div style={{ marginTop: 10 }}><div style={{ color: cyan, fontWeight: 950, fontSize: 13, marginBottom: 8 }}>層級規則</div><div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>{rs.map((r, i) => { const tier = `D${i + 1}`; const active = tier === activeTier; return <div key={tier} style={{ padding: "8px 3px", minHeight: 52, borderRadius: 12, textAlign: "center", background: active ? "rgba(49,231,255,.10)" : "rgba(8,18,35,.58)", border: active ? "1px solid rgba(49,231,255,.48)" : "1px solid rgba(148,163,184,.18)", color: active ? cyan : "#94a3b8", fontWeight: 900 }}><div>{tier}</div><div style={{ fontSize: 10 }}>{fmtPct(r).replace(".0", "")}</div><div style={{ fontSize: 10 }}>{fmtAmount(arr(amounts)[i] || 0)}</div></div>; })}</div></div>;
}
...
export function AssetCard({ row = {}, children }) {
  ...
  return <article style={{ padding: 14, borderRadius: 22, background: "radial-gradient(circle at 0 0,rgba(49,231,255,.18),transparent 36%),linear-gradient(135deg,rgba(11,19,36,.88),rgba(5,11,24,.78))", border: "1px solid rgba(49,231,255,.20)", color: "#f8fafc", boxShadow: "0 18px 40px rgba(0,0,0,.35)", backdropFilter: "blur(18px)" }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 10 }}><div style={{ display: "flex", gap: 8, alignItems: "center", minWidth: 0 }}><span style={{ flex: "0 0 auto", width: 15, height: 15, borderRadius: 999, background: "#35f59a", boxShadow: "0 0 16px rgba(53,245,154,.75)" }} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 21, fontWeight: 1000, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{row.symbol || "--"}</div><div style={{ color: "#94a3b8", fontWeight: 850, fontSize: 12 }}>{row.name || "--"}</div></div></div><strong style={{ flex: "0 0 auto", padding: "6px 10px", borderRadius: 999, background: bg, border: `1px solid ${border}`, color, fontSize: 13 }}>{TIER_ICON[row.tier] || row.tier || "D0"}</strong></div>
