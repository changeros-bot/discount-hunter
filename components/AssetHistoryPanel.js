...
export default function AssetHistoryPanel() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const historyRes = await fetch('/api/financial/asset-history?limit=30&t=' + Date.now(), { cache: 'no-store' });
  ...
  const chart = useMemo(() => {
    ...
    return { data, min, max, width, height, left, right, top, bottom, x, y, points };
  }, [rows]);
  ...
  return <section style={{ maxWidth: 430, margin: '0 auto', padding: '0 14px 130px', color: '#f8fafc', background: 'linear-gradient(180deg,#111827,#0f172a)' }}>
    <div style={{ background: 'linear-gradient(160deg,rgba(17,24,39,.97),rgba(8,18,34,.97))', border: '1px solid rgba(56,189,248,.42)', borderRadius: 22, padding: 16, boxShadow: '0 16px 40px rgba(0,0,0,.28)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 12, alignItems: 'center' }}>
        ...
        <b style={{ color: '#86efac', fontSize: 12 }}>近 {rows.length || 0} 天</b>
      </div>
      ...
      {chart ? <svg viewBox={`0 0 ${chart.width} ${chart.height}`} width="100%" role="img" aria-label="每日總資產變化曲線">
        ...
        {chart.data.map((item, index) => <g key={item.date}>
          <circle cx={chart.x(index)} cy={chart.y(Number(item.total || 0))} r="4" fill="#86efac"><title>{item.date}｜{nt(item.total)}</title></circle>
          {(index === 0 || index === chart.data.length - 1 || chart.data.length <= 7) && <text x={chart.x(index)} y={chart.height - 10} textAnchor="middle" fill="#94a3b8" fontSize="9">{item.date.slice(5)}</text>}
        </g>)}
      ...
      <div style={{ color: '#94a3b8', fontSize: 11, marginTop: 5 }}>每天保存一筆；同一天資產變動時更新當日快照，不重複新增。</div>
    </div>
  </section>;
}
