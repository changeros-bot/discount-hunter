...
def main():
    ...
    drop_cols = [c for c in ['sector_group','business_type','portfolio_role','strategy_bucket','stage'] if c in funds.columns]
    funds = funds.drop(columns=drop_cols)
    ...
    priority = {
        'PAPER_QUALITY_DISCOUNT': 1,
        'PAPER_DEEP_DISCOUNT': 2,
        'PAPER_GROWTH_DISCOUNT': 3,
        'PAPER_CYCLE_DISCOUNT': 4,
        'PAPER_INCOME_DISCOUNT': 5,
        'WAIT_QUALITY_PRICE': 6,
        'SECONDARY_QUALITY_REVIEW': 7,
        'CYCLE_WATCH': 8,
        'GROWTH_MONITOR': 9,
        'COMMODITY_CYCLE_MONITOR': 10,
        'INCOME_MONITOR': 11,
        'QUALITY_MONITOR': 12,
        'DEEP_DISCOUNT_MONITOR': 13,
        'VENTURE_MONITOR': 14,
        'SHORT_HISTORY_MONITOR': 15,
        'DATA_REPAIR': 16,
        'MONITOR': 17,
    }
    ...
    out.to_csv(ROOT / 'watchlist_36_group_decision_matrix.csv', index=False)
    ...
    counts = out.groupby(['strategy_bucket','decision']).size().reset_index(name='count')
    counts.to_csv(ROOT / 'watchlist_36_group_decision_counts.csv', index=False)

    md = ['# Watchlist 36 Group-Specific Decision Matrix', '', 'Research and paper-trading gates only. No live orders and no automatic roster changes.', '']
    for strategy, group in out.groupby('strategy_bucket', sort=True):
        md.append(f'## {strategy} ({len(group)})')
        for _, r in group.iterrows():
            md.append(f"- {r['ticker']} | {r['decision']} | Q{int(r['quality_points'])}/B{int(r['backtest_points'])} | {r['valuation_band']}")
        md.append('')
    (ROOT / 'watchlist_36_group_decision_summary.md').write_text('\n'.join(md), encoding='utf-8')
