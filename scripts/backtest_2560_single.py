#!/usr/bin/env python3
"""Generic single ticker 2560 backtest. Research only, no order execution."""
...
def clean(df):
    ...
    if date_col is None or close_col is None or volume_col is None:
        raise ValueError("needs date, close, volume")
...
def make_events(prices, ticker, industry):
    rows = []
    for i, row in prices.iterrows():
        p = pattern(row)
        if not p: continue
        event = {"ticker": ticker, "industry": industry, "pattern": p, "date": row.date, "close": row.close, "ma25": row.ma25, "vol5": row.vol5, "vol60": row.vol60, "above_ma200": row.above_ma200}
        for w in WINDOWS:
...
def entry_from_next_open(prices, signal_date, slippage):
    ...
    idx = idxs[0]; row = prices.iloc[idx]
    return idx, row.date, float(row.open) * (1 + slippage)


def simulate_trade(prices, signal_date, rule, cost, slippage):
    ...
    for d in range(1, max_days + 1):
        ...
        gross = close / entry_price - 1; reason = None
        if rule == "base_60d":
            ...
            if close < float(r.ma25): reason = "close_below_ma25"
            elif close <= top - 2 * float(r.atr14): reason = "atr_trail"
        if reason:
...
def load_prices(args):
    ...
    if raw.empty: raise ValueError(f"No yfinance data for {ticker}")
    return clean(flat(raw).reset_index())


def run(args):
    ticker = args.ticker.upper(); slug = ticker.lower().replace("-", "_")
    prices = load_prices(args)
    events = make_events(prices, ticker, args.industry)
    rows = []
    for _, event in events.iterrows():
        for rule in RULES:
            result = simulate_trade(prices, event.date, rule, args.cost, args.slippage)
            if result:
                rows.append({"ticker": ticker, "industry": args.industry, "pattern": event.pattern, "signal_date": event.date, "rule": rule, "cost": args.cost, "slippage": args.slippage, **result})
    trades = pd.DataFrame(rows); out = Path(args.output_dir); out.mkdir(parents=True, exist_ok=True)
    fmt(events).to_csv(out / f"{slug}_2560_events.csv", index=False)
    fmt(trades).to_csv(out / f"{slug}_2560_trades.csv", index=False)
    fmt(summarize(trades, ("rule",))).to_csv(out / f"{slug}_2560_summary.csv", index=False)
    fmt(summarize(trades, ("pattern", "rule"))).to_csv(out / f"{slug}_2560_by_pattern.csv", index=False)
    fmt(summarize(trades, ("rule", "exit_reason"))).to_csv(out / f"{slug}_2560_by_exit.csv", index=False)
    print(f"{ticker} 2560 complete. events={len(events)}, trades={len(trades)}")


def parse_args(argv: Optional[Sequence[str]] = None):
    ap = argparse.ArgumentParser()
    ap.add_argument("--ticker", required=True)
    ap.add_argument("--industry", default="AI半導體")
