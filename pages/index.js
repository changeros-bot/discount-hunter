...
const MODEL_VERSION = "15.41-dca-vs-dip-fix";
...
function formatCurrency(value, digits = 2) {
  ...
  const sign = r > 0 ? "+" : r < 0 ? "-" : "";
  return `${sign}$${Math.abs(r).toLocaleString(undefined, { maximumFractionDigits: digits })}`;
}
function formatPct(value, digits = 2) {
  const n = Number(value);
  if (!Number.isFinite(n)) return "--";
  const pct = Number((n * 100).toFixed(digits));
...
function getCompletedLevelByDipLedger() {
  // Monthly DCA holdings / existing wallet balances must not consume dip-buy layers.
...
function getNextBuyPoint(asset, completedLevel = 0) {
  ...
  return {
    ...
    progress,
    displayProgress: reached ? 100 : Math.min(99, Math.floor(progress)),
...
function ruleRows(asset) {
  const rules = asset.rules || [];
  const amounts = asset.amounts || [];
  return rules.map((rule, i) => ({ color: ruleColors[i] || "⚪", levelName: levelNames[i + 1] || `第${i + 1}層`, discountText: `-${Math.abs(parsePercentValue(rule) || 0)}%`, amountText: `${amounts[i] ?? 0}U` }));
}
...
export default function Home() {
  ...
  useEffect(() => {
    loadPrices();
    syncWallet();
    const p = setInterval(loadPrices, REFRESH_MS);
    const w = setInterval(syncWallet, 60000);
    return () => { clearInterval(p); clearInterval(w); };
  }, []);
  ...
  const enhancedAssets = useMemo(() => assets.map((asset) => {
    ...
    const completedLevel = getCompletedLevelByDipLedger();
  ...
  return <main className="page">
    <section className="hero compactHero" style={{ textAlign: "center", padding: "18px 12px 10px", background: "linear-gradient(135deg, rgba(10,14,39,.96), rgba(3,7,18,.96))" }}>
      <div style={{ textAlign: "right", color: "rgba(243,186,47,.6)", fontSize: 10, fontWeight: 900 }}>v15.41</div>
