...
function PaperRunCard({ run }) {
  const s = run?.summary || {};
  return <article style={{ marginTop: 8, padding: 10, borderRadius: 16, background: "rgba(2,6,23,.48)", border: "1px solid rgba(34,197,94,.30)" }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong style={{ color: "#f8fafc", fontSize: 16 }}>{timeTw(run?.createdAt)}</strong><Pill tone="green">已記錄</Pill></div>
    <div style={{ marginTop: 6, color: "#cbd5e1", fontSize: 12, lineHeight: 1.55, fontWeight: 850 }}>草稿：{s.draftCount || 0}｜人工確認：{s.wouldRequestManualConfirmationCount || 0}｜擋下：{s.wouldBlockCount || 0}｜金額：{Number(s.totalDraftAmountUsd || 0).toFixed(2)}U</div>
  </article>;
}
...
export default function AutoWhitelist() {
  ...
  const [paperAuto, setPaperAuto] = useState(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/v17/paper-auto-engine?t=${Date.now()}`, { cache: "no-store" }).then((r) => r.json()),
    ]).then(([w, d, p]) => {
  ...
  const latestPaper = paperAuto?.latestRun;
  const paperRuns = paperAuto?.runs || [];
  return <main style={{ minHeight: "100vh", color: "#f8fafc", background: "linear-gradient(180deg,#020617 0%,#07111f 55%,#0f172a 100%)", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans TC',Arial,sans-serif" }}>
    <div style={{ maxWidth: 460, margin: "0 auto", padding: "22px 14px 40px" }}>
      ...
      {data && <>
        <Box title="安全邊界" tone="yellow"><div><Pill tone="red">Auto Trade OFF</Pill><Pill>Paper Auto 7D</Pill><Pill>Dry Run ON</Pill><Pill tone="yellow">Kill Switch Required</Pill></div><div style={{ marginTop: 8, color: "#cbd5e1", fontWeight: 850, lineHeight: 1.55 }}>這裡只模擬今天如果開自動化會發生什麼，不會送出 Binance 訂單。</div></Box>
        <Box title="一週紙上自動化試跑" tone="green">
          <div><Pill>累計 {paperAuto?.runCount || 0} 次</Pill><Pill tone="green">最新：{timeTw(latestPaper?.createdAt)}</Pill></div>
          {paperRuns.length ? paperRuns.slice(0, 7).map((run) => <PaperRunCard key={run.id} run={run} />) : <div style={{ marginTop: 8, color: "#94a3b8", fontWeight: 850 }}>尚未有紙上自動化紀錄。排程啟動後會每天新增一筆。</div>}
        </Box>
        <Box title="今日即時乾跑" tone="green">
