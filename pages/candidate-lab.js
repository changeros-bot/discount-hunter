...
export default function CandidateLab() {
  ...
  return <main style={{ minHeight: "100vh", padding: 12, background: "radial-gradient(circle at 10% 0%,rgba(34,211,238,.12),transparent 28%),linear-gradient(180deg,#050b18,#020617)", color: "#f8fafc", fontFamily: "system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
    <header style={{ padding: 16, borderRadius: 22, background: "linear-gradient(135deg,rgba(8,47,73,.75),rgba(15,23,42,.96))", border: "1px solid rgba(34,211,238,.25)" }}>
      ...
      <div style={{ color: "#94a3b8", fontSize: 12, lineHeight: 1.6 }}>Stage 1：代號、Binance xStocks、報價、52 週高點與折價資料完整度。獨立於正式 10 檔及既有 28 檔紙上交易。</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 12 }}>
        ...
        <span style={{ padding: "6px 9px", borderRadius: 999, background: "rgba(30,41,59,.72)", color: "#cbd5e1", fontSize: 11, fontWeight: 900 }}>紙上建倉：尚未開始</span>
      </div>
    </header>
    ...
    <section style={{ marginTop: 12, padding: 14, borderRadius: 18, background: "rgba(15,23,42,.9)", border: "1px solid rgba(148,163,184,.16)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
        {[["總候選", data?.summary?.total || 0, "#f8fafc"], ["PASS", data?.summary?.pass || 0, "#86efac"], ["CHECK", data?.summary?.check || 0, "#fde68a"], ["FAIL", data?.summary?.fail || 0, "#fca5a5"], ["Binance", data?.summary?.binance || 0, "#67e8f9"], ["Fallback", data?.summary?.fallback || 0, "#c4b5fd"]].map(([label, value, color]) => <div key={label} style={{ padding: 10, borderRadius: 13, background: "rgba(2,6,23,.55)" }}><div style={{ color: "#94a3b8", fontSize: 10, fontWeight: 900 }}>{label}</div><div style={{ color, fontSize: 20, fontWeight: 1000, marginTop: 3 }}>{value}</div></div>)}
      </div>
      <button onClick={load} disabled={loading} style={{ width: "100%", marginTop: 10, padding: 11, borderRadius: 12, border: "1px solid rgba(34,211,238,.3)", background: "rgba(8,145,178,.18)", color: "#a5f3fc", fontWeight: 1000 }}>{loading ? "驗證中…" : "重新驗證全部候選"}</button>
        ...
        {items.map((row) => {
          ...
          return <article key={row.symbol} style={{ padding: 12, borderRadius: 16, background: "linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.96))", border: `1px solid ${s.border}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 7, marginTop: 10 }}>
              {[["現價", money(row.price)], ["52週高點", money(high)], ["折價", pct(row.discount)], ["訊號", `${row.signal?.text || "—"} ${row.signal?.amount || ""}`]].map(([k,v]) => <div key={k} style={{ padding: 8, borderRadius: 11, background: "rgba(2,6,23,.55)" }}><div style={{ color: "#7dd3fc", fontSize: 10, fontWeight: 900 }}>{k}</div><div style={{ marginTop: 3, color: "#f8fafc", fontWeight: 1000 }}>{v}</div></div>)}
            </div>
            <div style={{ marginTop: 8, color: "#cbd5e1", fontSize: 11, lineHeight: 1.55 }}>
              Provider：{row.quoteAudit?.provider || "—"}<br />
              Token：{row.tokenSymbol || "未辨識"}<br />
              狀態：{row.quoteAudit?.status || "—"}{row.quoteAudit?.error ? `｜${row.quoteAudit.error}` : ""}
            </div>
            <div style={{ marginTop: 8, color: "#64748b", fontSize: 10 }}>Stage 1 僅驗證資料可用性；尚未建立紙上部位。</div>
          </article>;
        })}
      </div>
    </section>)}
