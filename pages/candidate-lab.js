...
export default function CandidateLab() {
  ...
  return <main style={{ minHeight: "100vh", padding: 12, background: "radial-gradient(circle at 10% 0%,rgba(34,211,238,.12),transparent 28%),linear-gradient(180deg,#050b18,#020617)", color: "#f8fafc", fontFamily: "system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
    <header style={{ padding: 16, borderRadius: 22, background: "linear-gradient(135deg,rgba(8,47,73,.75),rgba(15,23,42,.96))", border: "1px solid rgba(34,211,238,.25)" }}>
      ...
      <div style={{ color: "#94a3b8", fontSize: 12, lineHeight: 1.6 }}>Stage 1 仍可查看即時資料；Stage 2 已暫停。正式觀測將在 Supabase 持久化資料庫接通後重新開始計時。</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 12 }}>
        ...
        <span style={{ padding: "6px 9px", borderRadius: 999, background: "rgba(248,113,113,.12)", color: "#fecaca", fontSize: 11, fontWeight: 900 }}>Stage 2：已暫停</span>
      </div>
    </header>
    ...
    <section style={{ marginTop: 12, padding: 14, borderRadius: 18, background: "rgba(15,23,42,.9)", border: "1px solid rgba(148,163,184,.16)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
        {[["總候選", data?.summary?.total || 0, "#f8fafc"], ["Stage1 PASS", data?.summary?.pass || 0, "#86efac"], ["Stage1 CHECK", data?.summary?.check || 0, "#fde68a"], ["Stage1 FAIL", data?.summary?.fail || 0, "#fca5a5"], ["正式快照", 0, "#67e8f9"], ["紙上部位", 0, "#c4b5fd"]].map(([label, value, color]) => <div key={label} style={{ padding: 10, borderRadius: 13, background: "rgba(2,6,23,.55)" }}><div style={{ color: "#94a3b8", fontSize: 10, fontWeight: 900 }}>{label}</div><div style={{ color, fontSize: 20, fontWeight: 1000, marginTop: 3 }}>{value}</div></div>)}
      </div>
      <div style={{ marginTop: 10, padding: 10, borderRadius: 12, background: "rgba(127,29,29,.15)", border: "1px solid rgba(248,113,113,.24)", color: "#fecaca", fontSize: 12, lineHeight: 1.6 }}>
        localStorage 舊快照已清除且不算正式資料。原因：無法由 GitHub Actions／伺服器排程自動寫入，也無法跨裝置持久保存。下一步只先建立 Supabase 的 candidate_validation_snapshots，並加入 triggered_by：manual／scheduled／github_actions。
      </div>
      <button onClick={loadStage1} disabled={loading} style={{ width: "100%", marginTop: 10, padding: 11, borderRadius: 12, border: "1px solid rgba(34,211,238,.3)", background: "rgba(8,145,178,.18)", color: "#a5f3fc", fontWeight: 1000 }}>{loading ? "讀取 Stage 1…" : "重新整理 Stage 1 即時資料"}</button>
      ...
      <div style={{ display: "grid", gap: 9 }}>
        {items.map((row) => {
          ...
          return <article key={row.symbol} style={{ padding: 12, borderRadius: 16, background: "linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.96))", border: `1px solid ${s.border}` }}>
            ...
            <div style={{ marginTop: 8, color: "#cbd5e1", fontSize: 11, lineHeight: 1.55 }}>Provider：{row.quoteAudit?.provider || "—"}<br />Token：{row.tokenSymbol || "未辨識"}<br />正式 Stage 2：等待 Supabase。</div>
          </article>;
        })}
      </div>
    </section>)}
