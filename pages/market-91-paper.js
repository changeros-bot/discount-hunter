...
function Row({ row, right, tone = "blue" }) {
  ...
  return <article style={{ marginTop: 8, padding: 10, borderRadius: 16, background: "rgba(2,6,23,.48)", border: `1px solid ${triggered ? "rgba(34,197,94,.34)" : "rgba(148,163,184,.18)"}` }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
      <strong style={{ color: "#f8fafc", fontSize: 18 }}>{row.symbol || row.name || "—"}</strong>
      <Pill tone={tone || (triggered ? "green" : "yellow")}>{right || (triggered ? "紙上買入" : "無動作")}</Pill>
    </div>
    <div style={{ marginTop: 6, color: "#cbd5e1", fontSize: 12, fontWeight: 850, lineHeight: 1.55 }}>
      {row.name ? <>{row.name}<br /></> : null}
      {row.role || row.bucket || "Paper Candidate"}｜跌幅：{row.discountText || "N/A"}｜價格：${Number(row.price || 0).toFixed(4)}
...
export default function Market91Paper() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      readJson(`/api/v17/market-91-shortlist?t=${Date.now()}`),
    ])
      .then(([paper, shortlist]) => setData({ paper, shortlist }))
      .catch((e) => setError(e.message || "讀取失敗"));
  }, []);
  const paper = data?.paper || {};
  const triggered = paper?.triggered || [];
  ...
  return <main style={{ minHeight: "100vh", color: "#f8fafc", background: "linear-gradient(180deg,#020617 0%,#07111f 55%,#0f172a 100%)", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans TC',Arial,sans-serif" }}>
    <div style={{ maxWidth: 460, margin: "0 auto", padding: "22px 14px 40px" }}>
      ...
      {!data && !error && <Box title="讀取中"><div style={{ color: "#94a3b8" }}>同步紙上交易狀態中…</div></Box>}
      ...
      {data && <>
        ...
        <Box title="紙上交易規則" tone="yellow">
          <div style={{ color: "#cbd5e1", lineHeight: 1.7, fontWeight: 850 }}>
            核心股：-15% / -25% / -35%，5U / 5U / 10U。<br />
            衛星股：-20% / -30% / -40%，5U / 5U / 10U。<br />
            高波動股：-25% / -40% / -55%，5U / 5U / 10U。
          </div>
        </Box>

        <Box title={`今日紙上買點（${triggered.length}）`} tone="green">
          {triggered.length ? triggered.map((row) => <Row key={row.symbol} row={row} right="紙上買入" tone="green" />) : <div style={{ color: "#94a3b8", fontWeight: 850 }}>今天沒有觸發紙上買點。</div>}
        </Box>

        <Box title={`核心觀察區（${coreRows.length}）`}>{coreRows.map((row) => <Row key={row.symbol} row={row} right="Core" tone="blue" />)}</Box>
        <Box title={`衛星觀察區（${satelliteRows.length}）`}>{satelliteRows.map((row) => <Row key={row.symbol} row={row} right="Satellite" tone="yellow" />)}</Box>
        <Box title={`高波動觀察區（${highVolRows.length}）`}>{highVolRows.map((row) => <Row key={row.symbol} row={row} right="High Vol" tone="red" />)}</Box>

        <Box title="入口">
          <LinkButton href="/v17">實盤折價獵人</LinkButton>
          <LinkButton href="/market-91-shortlist">17檔主名單</LinkButton>
          <LinkButton href="/trade-readiness">現金與預算檢查</LinkButton>
        </Box>
      </>}
    </div>
  </main>;
