...
const cats=['飲食','家用','服飾','交通','娛樂','醫療','金融','固定支出','生活用品','生活費','薪水','退稅','早餐','午餐','晚餐','飲料','咖啡','菸','點心/宵夜','機車','醫療健康','其他'];
const defaultBudgets=[{id:'b-living',name:'生活費',category:'生活費',amount:8000},{id:'b-fixed',name:'固定支出',category:'固定支出',amount:0}];
...
function payrollStart(){const d=new Date();return fmtDate(new Date(d.getFullYear(),d.getDate()>=10?d.getMonth():d.getMonth()-1,10))}
...
function cat(note){if(/家用/.test(note))return'生活費';if(/Chat/i.test(note))return'ChatGPT';if(/Google/i.test(note))return'Google One';if(/電信|手機/.test(note))return'手機月租費';if(/加油|停車|機車|罰單/.test(note))return'機車';if(/耳|鼻|醫|看耳朵|洗鼻器/.test(note))return'醫療健康';if(/富邦/.test(note))return'金融';if(/Badoo|足球/.test(note))return'娛樂';if(/雨衣/.test(note))return'服飾';if(/眼鏡|打火機/.test(note))return'生活用品';if(/感訓/.test(note))return'其他';if(/早餐/.test(note))return'早餐';if(/午餐|中餐/.test(note))return'午餐';if(/晚餐/.test(note))return'晚餐';if(/咖啡|冰美式|冰拿|熟拿|黑咖啡/.test(note))return'咖啡';if(/菸|芬/.test(note))return'菸';if(/飲料/.test(note))return'飲料';if(/豆|酥|麵包|挫冰|雞排|冰/.test(note))return'點心/宵夜';return'其他'}
function isHousehold(t){return /家用/.test(t.note||'')||t.account==='家用'}
function groupTx(t){const c=t.category;if(isHousehold(t))return'家用';if(food.includes(c))return'飲食';if(c==='機車')return'交通';if(c==='醫療健康')return'醫療';if(fixed.includes(c))return'固定支出';if(['服飾','娛樂','金融','生活用品','交通','醫療'].includes(c))return c;return'其他'}
function living(c){return food.includes(c)||['生活用品','機車','生活費'].includes(c)}
function sortTx(rows){return [...rows].sort((a,b)=>(b.date||'').localeCompare(a.date||'')||String(b.id||'').localeCompare(String(a.id||'')))}
const seed=[...expenseCsv.split('\n').map((l,i)=>{const [date,note,amount]=l.split(',');const c=cat(note);return{id:'e'+i+date+note,date,type:'支出',amount:+amount,account:'自用',category:c,note}}),{id:'income-salary-202606',date:'2026-06-30',type:'收入',amount:50350,account:'自用',category:'薪水',note:'薪水'}];
function cleanDb(rows){return Array.isArray(rows)?rows.filter(t=>t&&t.id&&!(t.id==='income-tax-202606'||(t.date==='2026-06-30'&&t.type==='收入'&&t.category==='退稅'&&Number(t.amount)===1300))):[]}
function stat(txs,r){const tx=txs.filter(t=>t.date>=r.start&&t.date<=r.end),ex=tx.filter(t=>t.type==='支出'),inc=tx.filter(t=>t.type==='收入');const sum=a=>a.reduce((s,x)=>s+Number(x.amount||0),0);const expense=sum(ex),income=sum(inc),lifeRows=ex.filter(t=>living(t.category)&&!isHousehold(t)),fixedRows=ex.filter(t=>fixed.includes(t.category));const groups=['飲食','家用','服飾','交通','娛樂','醫療','金融','固定支出','生活用品','其他'].map(name=>({name,amount:sum(ex.filter(t=>groupTx(t)===name))})).filter(x=>x.amount>0);const foods=food.map(name=>({name,amount:sum(ex.filter(t=>t.category===name))})).filter(x=>x.amount>0);return{tx,ex,inc,expense,income,balance:income-expense,life:sum(lifeRows),fix:sum(fixedRows),lifeRows,fixedRows,groups,foods}}
function budgetSpent(b,s){if(b.category==='生活費')return s.life;if(b.category==='固定支出')return s.fix;return s.ex.filter(t=>groupTx(t)===b.category||t.category===b.category).reduce((a,b)=>a+Number(b.amount||0),0)}
...
function Chart({rows,total,onRow}){const base=Math.max(1,total||rows.reduce((s,x)=>s+x.amount,0));return <div style={{display:'grid',gap:12}}>{rows.map(x=><div key={x.name} onClick={()=>onRow&&onRow(x)} style={{display:'grid',gridTemplateColumns:'70px 1fr 76px',gap:10,alignItems:'center',cursor:onRow?'pointer':'default',padding:'2px 0'}}><b>{x.name}</b><Bar amount={x.amount} total={base} danger={x.name==='菸'}/><b style={{textAlign:'right'}}>{nt(x.amount)}</b><span style={{gridColumn:'2 / 3',color:'#94a3b8',fontSize:10,marginTop:-8}}>{(x.amount/base*100).toFixed(1)}%｜點開</span></div>)}</div>}
function DetailSheet({detail,onClose}){if(!detail)return null;const rows=sortTx(detail.rows);return <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:50,background:'rgba(0,0,0,.55)',display:'grid',alignItems:'end'}}><section onClick={e=>e.stopPropagation()} style={{maxWidth:430,width:'100%',margin:'0 auto',maxHeight:'76vh',overflow:'auto',background:'linear-gradient(160deg,#0f172a,#020617)',border:'1px solid rgba(34,197,94,.42)',borderRadius:'24px 24px 0 0',padding:18,boxShadow:'0 -20px 60px rgba(34,197,94,.18)'}}><div style={{display:'flex',justifyContent:'space-between',gap:10,marginBottom:10}}><div><h2 style={{margin:'0 0 6px',fontSize:22}}>{detail.title}</h2><div style={{fontSize:34,fontWeight:1000,color:detail.bad?'#ff5252':'#86efac'}}>{nt(detail.total)}</div><p style={{color:'#94a3b8',fontSize:12,lineHeight:1.6,margin:'8px 0 0'}}>{detail.sub}</p></div><button onClick={onClose} style={{border:'1px solid rgba(148,163,184,.25)',background:'rgba(15,23,42,.8)',color:'#e2e8f0',borderRadius:999,padding:'8px 12px',fontWeight:900}}>關閉</button></div><div style={{color:'#94a3b8',fontSize:12,fontWeight:900,margin:'12px 0'}}>共 {rows.length} 筆｜新到舊</div>{rows.map(t=><div key={t.id} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:10,padding:'11px 0',borderBottom:'1px solid rgba(148,163,184,.12)'}}><div><b>{t.note}</b><div style={{color:'#94a3b8',fontSize:12,marginTop:4}}>{t.date}｜{t.category}｜{t.account}｜{t.type}</div></div><b style={{color:t.type==='收入'?'#86efac':'#fca5a5'}}>{nt(t.amount)}</b></div>)}</section></div>}
function Dashboard({txs}){const [r,setR]=useState({start:monthStart(),end:today()}),[detail,setDetail]=useState(null);const s=stat(txs,r),remain=8000-s.life,rows=sortTx(s.tx),foodTotal=s.foods.reduce((a,b)=>a+b.amount,0),period=`${r.start.replaceAll('-','/')} - ${r.end.replaceAll('-','/')}`;const show=(title,total,rows,bad=false,sub='')=>setDetail({title,total,bad,sub:sub||`${period}｜共 ${rows.length} 筆`,rows});return <><Card><Title t='日期篩選' r='Date Range'/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><input type='date' value={r.start} onChange={e=>setR({...r,start:e.target.value})} style={input()}/><input type='date' value={r.end} onChange={e=>setR({...r,end:e.target.value})} style={input()}/></div><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8,marginTop:10}}><Btn onClick={()=>setR({start:monthStart(),end:today()})}>本月</Btn><Btn onClick={()=>setR({start:'2026-06-01',end:'2026-06-30'})}>六月</Btn><Btn onClick={()=>setR({start:'2026-06-01',end:today()})}>6月起</Btn><Btn onClick={()=>setR({start:payrollStart(),end:today()})}>發薪後</Btn><Btn onClick={()=>setR({start:'2026-07-01',end:today()})}>七月</Btn></div><p style={{color:'#94a3b8',fontWeight:800}}>Local DB｜統計區間：{period}</p></Card><Card onClick={()=>show('區間結餘',s.balance,[...s.inc,...s.ex],s.balance<0,`${period}｜收入 ${nt(s.income)} - 支出 ${nt(s.expense)}`)} style={{padding:20,borderColor:s.balance>=0?'rgba(34,197,94,.7)':'rgba(255,82,82,.6)'}}><Title t='區間結餘' r='點開明細'/><div style={{fontSize:42,fontWeight:1000,color:s.balance>=0?'#86efac':'#ff5252'}}>{nt(s.balance)}</div><p style={{color:'#94a3b8'}}>收入 {nt(s.income)} - 支出 {nt(s.expense)}｜收入 {s.inc.length} 筆，支出 {s.ex.length} 筆</p></Card><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}><Metric onClick={()=>show('區間收入',s.income,s.inc,false,`${period}｜收入 ${s.inc.length} 筆`)} label='區間收入' value={nt(s.income)} sub={`${s.inc.length} 筆收入`} good={s.income>0}/><Metric onClick={()=>show('區間支出',s.expense,s.ex,true,`${period}｜支出 ${s.ex.length} 筆`)} label='區間支出' value={nt(s.expense)} sub={`${s.ex.length} 筆支出`}/><Metric onClick={()=>show('個人生活費',s.life,s.lifeRows,remain<0,`排除家用｜預算 $8,000｜${remain<0?'超支 '+nt(Math.abs(remain)):'剩餘 '+nt(remain)}`)} label='生活費' value={nt(s.life)} sub={remain<0?'超支 '+nt(Math.abs(remain)):'剩餘 '+nt(remain)} bad={remain<0}/><Metric onClick={()=>show('固定支出',s.fix,s.fixedRows,false,`${period}｜固定支出 ${s.fixedRows.length} 筆`)} label='固定支出' value={nt(s.fix)} sub='手機 / 訂閱 / 保險'/></div><Card><Title t='區間大類支出' r='占總支出｜可點開'/><Chart rows={s.groups} total={s.expense} onRow={x=>show(`${x.name}支出`,x.amount,s.ex.filter(t=>groupTx(t)===x.name),true,`${period}｜${x.name}｜占總支出 ${(x.amount/Math.max(1,s.expense)*100).toFixed(1)}%`)}/></Card><Card><Title t='飲食細項' r={'占飲食 '+nt(foodTotal)+'｜可點開'}/><Chart rows={s.foods} total={foodTotal} onRow={x=>show(`${x.name}明細`,x.amount,s.ex.filter(t=>t.category===x.name),x.name==='菸',`${period}｜${x.name}`)}/></Card><Card><details><summary style={{cursor:'pointer',fontWeight:900}}>區間交易紀錄：{rows.length} 筆｜新到舊</summary>{rows.map(t=><div key={t.id} style={{display:'grid',gridTemplateColumns:'1fr auto',padding:'10px 0',borderBottom:'1px solid rgba(148,163,184,.12)'}}><div><b>{t.note}</b><div style={{color:'#94a3b8',fontSize:12}}>{t.date}｜{t.category}｜{t.type}</div></div><b style={{color:t.type==='收入'?'#86efac':'#fca5a5'}}>{nt(t.amount)}</b></div>)}</details></Card><DetailSheet detail={detail} onClose={()=>setDetail(null)}/></>}
function Entry({txs,setTxs,resetDb,exportDb}){const [f,setF]=useState({date:today(),type:'支出',amount:'',account:'自用',category:'飲食',note:''});const sortedTxs=sortTx(txs);function save(){const amount=Number(f.amount||0);if(!amount)return;const c=f.type==='收入'?f.category:(f.category==='飲食'?cat(f.note):f.category);setTxs([{id:'m'+Date.now(),...f,amount,category:c,note:f.note||c},...txs]);setF({...f,amount:'',note:''})}return <><Card><Title t='快速記帳' r='Local DB'/><div style={{display:'grid',gap:10}}><input value={f.amount} onChange={e=>setF({...f,amount:e.target.value})} style={{...input(),fontSize:22,fontWeight:900}} placeholder='金額' inputMode='numeric'/><input value={f.note} onChange={e=>setF({...f,note:e.target.value})} style={input()} placeholder='備註：早餐 / 薪水 / 加油 / 家用'/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><input type='date' value={f.date} onChange={e=>setF({...f,date:e.target.value})} style={input()}/><select value={f.type} onChange={e=>setF({...f,type:e.target.value})} style={input()}>{['支出','收入'].map(x=><option key={x}>{x}</option>)}</select></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><select value={f.account} onChange={e=>setF({...f,account:e.target.value})} style={input()}>{['家用','自用','投資'].map(x=><option key={x}>{x}</option>)}</select><select value={f.category} onChange={e=>setF({...f,category:e.target.value})} style={input()}>{cats.map(x=><option key={x}>{x}</option>)}</select></div><Btn onClick={save}>儲存這筆</Btn></div></Card><Card><Title t='資料庫操作' r='備份 / 重置'/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><Btn onClick={exportDb}>匯出 JSON</Btn><Btn onClick={resetDb}>重置內建</Btn></div></Card><Card><Title t='交易清單' r={`${txs.length} 筆｜新到舊`}/>{sortedTxs.map(t=><div key={t.id} style={{display:'grid',gridTemplateColumns:'1fr auto auto',gap:10,padding:'10px 0',borderBottom:'1px solid rgba(148,163,184,.12)'}}><div><b>{t.note}</b><div style={{color:'#94a3b8',fontSize:12}}>{t.date}｜{t.category}｜{t.type}</div></div><b>{nt(t.amount)}</b><button onClick={()=>setTxs(txs.filter(x=>x.id!==t.id))} style={{background:'transparent',color:'#fca5a5',border:'1px solid rgba(248,113,113,.3)',borderRadius:10}}>刪</button></div>)}</Card></>}
function Budget({txs,budgets,setBudgets}){const [form,setForm]=useState({name:'',category:'生活費',amount:''});const s=stat(txs,{start:monthStart(),end:today()});function addBudget(){const amount=Number(form.amount||0);if(!amount)return;setBudgets([{id:'b'+Date.now(),name:form.name||form.category,category:form.category,amount},...budgets]);setForm({name:'',category:'生活費',amount:''})}return <><Card><Title t='新增預算' r='Local DB'/><div style={{display:'grid',gap:10}}><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} style={input()} placeholder='預算名稱，例如：機車維修'/><select value={form.category} onChange={e=>setForm({...form,category:e.target.value})} style={input()}>{cats.filter(x=>!['薪水','退稅'].includes(x)).map(x=><option key={x}>{x}</option>)}</select><input value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} style={input()} placeholder='預算金額' inputMode='numeric'/><Btn onClick={addBudget}>新增預算</Btn></div></Card>{budgets.map(b=>{const spent=budgetSpent(b,s),remain=Number(b.amount||0)-spent,rate=b.amount?Math.round(spent/b.amount*100):0;return <Card key={b.id}><Title t={b.name} r='本月'/><Metric label={b.category} value={nt(spent)} sub={`${remain<0?'超支 '+nt(Math.abs(remain)):'剩餘 '+nt(remain)}｜預算 ${nt(b.amount)}｜${rate}%`} bad={remain<0}/><button onClick={()=>setBudgets(budgets.filter(x=>x.id!==b.id))} style={{marginTop:12,background:'transparent',color:'#fca5a5',border:'1px solid rgba(248,113,113,.3)',borderRadius:10,padding:'8px 10px',fontWeight:900}}>刪除預算</button></Card>})}</>}
function Assets(){const assets=[{id:'a1',name:'合作金庫（自用）',amount:76,note:'自用'},{id:'a2',name:'富邦銀行（投資）',amount:3299,note:'約當台幣'},{id:'a3',name:'郵局存款（家用）',amount:17152,note:'手動輸入'},{id:'a4',name:'現金',amount:0,note:'手動輸入'}];const hunter={total:94.37};const total=assets.reduce((s,a)=>s+Number(a.amount||0),0),grand=total+hunter.total*RATE;return <><Card><Title t='綜合總資產' r='TWD 估算'/><div style={{fontSize:42,fontWeight:1000}}>{nt(grand)}</div><p style={{color:'#94a3b8'}}>手動資產 {nt(total)} + 獵人投資 {usd(hunter.total)} × 匯率 {RATE}</p></Card><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}><Metric label='手動資產' value={nt(total)} sub='銀行約當台幣'/><Metric label='獵人投資市值' value={usd(hunter.total)} sub='BTC + xStocks'/></div><Card><Title t='手動資產清單' r={`${assets.length} 筆`}/>{assets.map(a=><div key={a.id} style={{display:'grid',gridTemplateColumns:'1fr auto',padding:'10px 0',borderBottom:'1px solid rgba(148,163,184,.12)'}}><div><b>{a.name}</b><div style={{color:'#94a3b8',fontSize:12}}>{a.note}</div></div><b style={{color:'#86efac'}}>{nt(a.amount)}</b></div>)}</Card></>}
export default function FinancialOSPage(){const [tab,setTab]=useState('dashboard'),[txs,setTxs]=useState([]),[budgets,setBudgets]=useState(defaultBudgets),[loaded,setLoaded]=useState(false);useEffect(()=>{try{const raw=localStorage.getItem(TX_KEY)||localStorage.getItem(OLD_TX_KEY);setTxs(raw?cleanDb(JSON.parse(raw)):seed)}catch{setTxs(seed)}try{const b=JSON.parse(localStorage.getItem(BUDGET_KEY)||'null');if(Array.isArray(b))setBudgets(b)}catch{}setLoaded(true)},[]);useEffect(()=>{if(loaded)localStorage.setItem(TX_KEY,JSON.stringify(cleanDb(txs)))},[txs,loaded]);useEffect(()=>{if(loaded)localStorage.setItem(BUDGET_KEY,JSON.stringify(budgets))},[budgets,loaded]);function resetDb(){if(confirm('確定重置為內建資料？這會覆蓋目前本機交易資料。'))setTxs(seed)}function exportDb(){const blob=new Blob([JSON.stringify({version:'financial-os-v3.8',exportedAt:new Date().toISOString(),transactions:txs,budgets},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`josh-financial-os-${today()}.json`;a.click();URL.revokeObjectURL(url)}const tabs=[['dashboard','總覽'],['entry','記帳'],['budget','預算'],['assets','資產']];return <main style={{minHeight:'100vh',color:'#f8fafc',background:'radial-gradient(circle at 50% -10%,rgba(34,197,94,.16),transparent 28%),linear-gradient(180deg,#020617,#0f172a 55%,#111827)',fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans TC',Arial,sans-serif"}}><div style={{maxWidth:430,margin:'0 auto',padding:'18px 14px 130px'}}><section style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12,marginBottom:14}}><div><div style={{fontSize:22,fontWeight:1000}}>Josh Financial OS</div><div style={{color:'#94a3b8',fontSize:12,fontWeight:800,marginTop:5}}>多元記帳本 V3.8｜家用分類｜新增預算</div></div><a href='/josh-os' style={{color:'#bbf7d0',textDecoration:'none',border:'1px solid rgba(34,197,94,.42)',borderRadius:999,padding:'7px 10px',fontSize:12,fontWeight:950}}>四合一</a></section>{tab==='dashboard'&&<Dashboard txs={txs}/>} {tab==='entry'&&<Entry txs={txs} setTxs={setTxs} resetDb={resetDb} exportDb={exportDb}/>} {tab==='budget'&&<Budget txs={txs} budgets={budgets} setBudgets={setBudgets}/>} {tab==='assets'&&<Assets/>}</div><nav style={{position:'fixed',left:0,right:0,bottom:0,background:'rgba(2,6,23,.92)',backdropFilter:'blur(16px)',borderTop:'1px solid rgba(34,197,94,.24)',padding:'8px 10px 10px'}}><div style={{maxWidth:430,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6}}>{tabs.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{border:'none',borderRadius:12,padding:'9px 4px',background:tab===k?'rgba(34,197,94,.16)':'transparent',color:tab===k?'#f8fafc':'#94a3b8',fontSize:11,fontWeight:900}}>{l}</button>)}</div></nav></main>}
