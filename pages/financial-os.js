...
function payrollStart(){const d=new Date();const s=new Date(d.getFullYear(),d.getDate()>=10?d.getMonth():d.getMonth()-1,10);return fmtDate(s)}
...
export default function FinancialOSPage(){const [tab,setTab]=useState('dashboard');const [txs,setTxs]=useState([]);const [loaded,setLoaded]=useState(false);useEffect(()=>{try{const raw=localStorage.getItem(TX_KEY)||localStorage.getItem(OLD_TX_KEY);if(raw){const data=cleanDb(JSON.parse(raw));setTxs(data.length?data:seed)}else{setTxs(seed)}}catch{setTxs(seed)}setLoaded(true)},[]);useEffect(()=>{if(!loaded)return;try{localStorage.setItem(TX_KEY,JSON.stringify(cleanDb(txs)))}catch{}},[txs,loaded]);function resetDb(){if(confirm('確定重置為內建資料？這會覆蓋目前本機交易資料。'))setTxs(seed)}function exportDb(){const blob=new Blob([JSON.stringify({version:'financial-os-v3.7',exportedAt:new Date().toISOString(),transactions:txs},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`josh-financial-os-${today()}.json`;a.click();URL.revokeObjectURL(url)}const tabs=[['dashboard','總覽'],['entry','記帳'],['budget','預算'],['assets','資產']];return <main style={{minHeight:'100vh',color:'#f8fafc',background:'radial-gradient(circle at 50% -10%,rgba(34,197,94,.16),transparent 28%),linear-gradient(180deg,#020617,#0f172a 55%,#111827)',fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans TC',Arial,sans-serif"}}><div style={{maxWidth:430,margin:'0 auto',padding:'18px 14px 130px'}}><section style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12,marginBottom:14}}><div><div style={{fontSize:22,fontWeight:1000}}>Josh Financial OS</div><div style={{color:'#94a3b8',fontSize:12,fontWeight:800,marginTop:5}}>多元記帳本 V3.7｜交易清單新到舊｜Local DB</div></div><a href='/josh-os' style={{color:'#bbf7d0',textDecoration:'none',border:'1px solid rgba(34,197,94,.42)',borderRadius:999,padding:'7px 10px',fontSize:12,fontWeight:950}}>四合一</a></section>{tab==='dashboard'&&<Dashboard txs={txs}/>} {tab==='entry'&&<Entry txs={txs} setTxs={setTxs} resetDb={resetDb} exportDb={exportDb}/>} {tab==='budget'&&<Budget txs={txs}/>} {tab==='assets'&&<Assets/>}</div><nav style={{position:'fixed',left:0,right:0,bottom:0,background:'rgba(2,6,23,.92)',backdropFilter:'blur(16px)',borderTop:'1px solid rgba(34,197,94,.24)',padding:'8px 10px 10px'}}><div style={{maxWidth:430,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6}}>{tabs.map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{border:'none',borderRadius:12,padding:'9px 4px',background:tab===k?'rgba(34,197,94,.16)':'transparent',color:tab===k?'#f8fafc':'#94a3b8',fontSize:11,fontWeight:900}}>{l}</button>)}</div></nav></main>}
