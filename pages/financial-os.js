...
const TX_KEY='josh-financial-os-v31-tx';
const RATE=32.5;
...
function stat(txs,r){const tx=txs.filter(t=>t.date>=r.start&&t.date<=r.end);const ex=tx.filter(t=>t.type==='支出'),inc=tx.filter(t=>t.type==='收入');const sum=a=>a.reduce((s,x)=>s+Number(x.amount||0),0);const expense=sum(ex),income=sum(inc),life=sum(ex.filter(t=>living(t.category))),fix=sum(ex.filter(t=>fixed.includes(t.category)));const groups=['飲食','服飾','交通','娛樂','醫療','金融','固定支出','生活用品','其他'].map(name=>({name,amount:sum(ex.filter(t=>group(t.category)===name))})).filter(x=>x.amount>0);const foods=food.map(name=>({name,amount:sum(ex.filter(t=>t.category===name))})).filter(x=>x.amount>0);return{tx,ex,inc,expense,income,balance:income-expense,life,fix,groups,foods}}
...
function Dashboard({txs}){const [r,setR]=useState({start:monthStart(),end:today()});const s=stat(txs,r);const remain=8000-s.life;const rows=[...s.tx].sort((a,b)=>b.date.localeCompare(a.date));const foodTotal=s.foods.reduce((a,b)=>a+b.amount,0);return <><Card><Title t='日期篩選' r='Date Range'/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><input type='date' value={r.start} onChange={e=>setR({...r,start:e.target.value})} style={input()}/><input type='date' value={r.end} onChange={e=>setR({...r,end:e.target.value})} style={input()}/></div><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8,marginTop:10}}><Btn onClick={()=>setR({start:monthStart(),end:today()})}>本月</Btn><Btn onClick={()=>setR({start:'2026-06-01',end:'2026-06-30'})}>六月</Btn><Btn onClick={()=>setR({start:'2026-06-01',end:today()})}>6月起</Btn><Btn onClick={()=>setR({start:payrollStart(),end:today()})}>發薪後</Btn><Btn onClick={()=>setR({start:'2026-07-01',end:today()})}>七月</Btn></div><p style={{color:'#94a3b8',fontWeight:800}}>預設：每月1號至今｜發薪後：每月10號起算</p><p style={{color:'#94a3b8',fontWeight:800}}>統計區間：{r.start.replaceAll('-','/')} - {r.end.replaceAll('-','/')}</p></Card><Card style={{padding:20,borderColor:s.balance>=0?'rgba(34,197,94,.7)':'rgba(255,82,82,.6)'}}><Title t='區間結餘' r={s.balance>=0?'正現金流':'負現金流'}/><div style={{fontSize:42,fontWeight:1000,color:s.balance>=0?'#86efac':'#ff5252'}}>{nt(s.balance)}</div><p style={{color:'#94a3b8'}}>收入 {nt(s.income)} - 支出 {nt(s.expense)}｜收入 {s.inc.length} 筆，支出 {s.ex.length} 筆</p></Card><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}><Metric label='區間收入' value={nt(s.income)} sub={`${s.inc.length} 筆收入`} good={s.income>0}/><Metric label='區間支出' value={nt(s.expense)} sub={`${s.ex.length} 筆支出`}/><Metric label='生活費' value={nt(s.life)} sub={remain<0?'超支 '+nt(Math.abs(remain)):'剩餘 '+nt(remain)} bad={remain<0}/><Metric label='固定支出' value={nt(s.fix)} sub='手機 / 訂閱 / 保險'/></div><Card><Title t='區間大類支出' r='占總支出'/><Chart rows={s.groups} total={s.expense}/></Card><Card><Title t='飲食細項' r={'占飲食 '+nt(foodTotal)}/><Chart rows={s.foods} total={foodTotal}/></Card><Card><details><summary style={{cursor:'pointer',fontWeight:900}}>區間交易紀錄：{rows.length} 筆</summary>{rows.map(t=><div key={t.id} style={{display:'grid',gridTemplateColumns:'1fr auto',padding:'10px 0',borderBottom:'1px solid rgba(148,163,184,.12)'}}><div><b>{t.note}</b><div style={{color:'#94a3b8',fontSize:12}}>{t.date}｜{t.category}｜{t.type}</div></div><b style={{color:t.type==='收入'?'#86efac':'#fca5a5'}}>{nt(t.amount)}</b></div>)}</details></Card></>}
...
function Budget({txs}){const s=stat(txs,{start:monthStart(),end:today()});return <><Card><Title t='生活費預算' r='本月'/><Metric label='生活費' value={nt(s.life)} sub={s.life>8000?'超支 '+nt(s.life-8000):'剩餘 '+nt(8000-s.life)} bad={s.life>8000}/></Card><Card><Title t='固定支出' r='本月'/><Metric label='固定支出' value={nt(s.fix)} sub='手機 / 訂閱 / 保險'/></Card></>}
