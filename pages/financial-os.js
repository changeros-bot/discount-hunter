...
const STORAGE_KEY="josh-financial-os-transactions-20260705-v9";
...
const seedBudgets=[{id:"budget-living",name:"生活費分類",category:"生活費",amount:8000}];
...
function assetValue(a){const note=`${a.note||""} ${a.name||""}`;const twd=(note.match(/(?:台幣|TWD)\s*(\d+(?:\.\d+)?)/i)||[])[1];const usdv=(note.match(/(?:美金|USD)\s*(\d+(?:\.\d+)?)/i)||[])[1];if(twd||usdv)return num(twd)+num( usdv)*USD_TWD_RATE;return num(a.amount)}
function assetSub(a){const note=`${a.note||""}`;return /美金|USD/.test(note)?`${a.type}｜${note}｜已換算`: `${a.type}｜${note||"手動輸入"}`}
const seedTransactions=rawRows.split("\n").map((line,i)=>{const[date,note,amount]=line.split("|");const category=cat(note);return{id:`seed-${date}-${i}-${note}`,date,type:"支出",amount:num(amount),account:"自用",category,note,isLivingExpense:category==="生活費"?"Y":"N",isFixedExpense:isFixed(category)?"Y":"N",affectsBudget:"Y"}});
function stats(transactions,range){const tx=transactions.filter(t=>inRange(t,range)),expenses=tx.filter(t=>t.type==="支出"),income=sum(tx.filter(t=>t.type==="收入"));const expense=sum(expenses),livingClass=sum(expenses.filter(t=>t.category==="生活費")),fixed=sum(expenses.filter(t=>t.isFixedExpense==="Y"||isFixed(t.category)));const groups=["飲食","服飾","居住","交通","教育","娛樂","醫療","金融","固定支出","生活用品","其他"].map(name=>({name,amount:sum(expenses.filter(t=>groupOf(t.category)===name))})).filter(x=>x.amount>0);const foods=FOOD_CATEGORIES.map(name=>({name,amount:sum(expenses.filter(t=>t.category===name))}));return{tx,expenses,income,expense,livingClass,fixed,groups,foods}}
function spentForBudget(b,s){if(b.category==="生活費")return s.livingClass;if(b.category==="固定支出")return s.fixed;return sum(s.expenses.filter(t=>groupOf(t.category)===b.category||t.category===b.category))}
