...
const cleanDb = (rows) => Array.isArray(rows) ? rows.filter((t) => t && t.id && !(t.id === 'income-tax-202606' || (t.date === '2026-06-30' && t.type === '收入' && t.category === '退稅' && Number(t.amount) === 1300))).map((t) => ({ ...t, account: t.account || '自用', budgetId: t.budgetId || '' })) : [];
...
const cleanBudgets = (rows) => Array.isArray(rows) ? rows.filter((b) => b && b.id && b.name).map((b) => ({ ...b, amount: Number(b.amount || 0), mode: b.id === 'b-living' ? 'living_cycle' : b.id === 'b-invest' ? 'investment' : 'project' })) : defaultBudgets;
...
function stat(txs, r) {
  ...
  return { tx, ex, inc, invRows, income, expense, invest, totalOutflow, balance: income - totalOutflow, life: sum(lifeRows), fix: sum(fixedRows), lifeRows, fixedRows, groups, outflowGroups, salaryTransfer };
}
...
function Dashboard({ txs }) {
  ...
  return <>
    ...
    <Card><Title t="可用結餘" r="Local DB" /><div style={{ fontSize: 42, fontWeight: 1000, color: s.balance >= 0 ? '#86efac' : '#ff5252' }}>{nt(s.balance)}</div><p style={{ color: '#94a3b8' }}>收入 {nt(s.income)} - 總支出 {nt(s.totalOutflow)}（一般 {nt(s.expense)} + 投資 {nt(s.invest)}）</p></Card>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}><Metric label="區間收入" value={nt(s.income)} good={s.income > 0} /><Metric label="薪轉收入" value={nt(s.salaryTransfer)} good={s.salaryTransfer > 0} /><Metric label="區間總支出" value={nt(s.totalOutflow)} sub={`一般支出 ${nt(s.expense)}｜投資 ${nt(s.invest)}`} /><Metric label="投資扣款" value={nt(s.invest)} /><Metric label="生活費（10日週期）" value={nt(livingStats.life)} sub={`${cycle.start}～${cycle.end}｜${remain < 0 ? `超支 ${nt(Math.abs(remain))}` : `剩餘 ${nt(remain)}`}`} bad={remain < 0} /></div>
...
function Entry({ txs, setTxs, budgets }) {
  ...
  function save() {
    ...
    setTxs([{ id: 'm' + Date.now(), ...form, account, amount, category, note: form.note || category, budgetId: form.type === '支出' ? form.budgetId : '' }, ...txs]);
    setForm({ ...form, amount: '', note: '', budgetId: '' });
  }
  return <>
    ...
    <Card><Title t="交易清單" r={`${txs.length} 筆`} /><TxList rows={txs} budgets={budgets} /></Card>
  </>;
}
...
function Budget({ txs, budgets, setBudgets }) {
  ...
  function budgetRows(b) {
    if (b.id === 'b-living' || b.mode === 'living_cycle') return livingStats.lifeRows;
    if (b.id === 'b-invest' || b.mode === 'investment') return monthStats.invRows;
    return txs.filter((t) => t.type === '支出' && t.budgetId === b.id);
  }
  function spent(b) { return budgetRows(b).reduce((sum, t) => sum + Number(t.amount || 0), 0); }
  function periodLabel(b) {
    if (b.id === 'b-living' || b.mode === 'living_cycle') return `${cycle.start.slice(5)}～${cycle.end.slice(5)}`;
    if (b.id === 'b-invest' || b.mode === 'investment') return '本月';
    return '專案累計';
  }
  ...
  function startEdit(b) { setEditingId(b.id); setEditForm({ name: b.name, category: b.category, amount: String(b.amount ?? 0) }); }
  function cancelEdit() { setEditingId(''); setEditForm(emptyBudget); }
  function saveEdit(id) { const amount = Number(editForm.amount); if (!editForm.name.trim() || !Number.isFinite(amount) || amount < 0) return; setBudgets(budgets.map((b) => b.id === id ? { ...b, name: editForm.name.trim(), category: editForm.category, amount } : b)); cancelEdit(); }
  function deleteBudget(id) { setBudgets(budgets.filter((b) => b.id !== id)); if (editingId === id) cancelEdit(); }
  ...
  return <>
    ...
    <Card style={{ borderColor: 'rgba(56,189,248,.35)' }}><div style={{ color: '#bae6fd', fontSize: 12, fontWeight: 900, lineHeight: 1.6 }}>計算規則：生活費固定每月10日～次月9日；地磚、燈具、學費、筆電等專案，只計入記帳時明確指定該專案的支出，不再用「家用／教育／其他」分類自動灌入。</div></Card>
    {budgets.map((b) => { const rows = budgetRows(b); const used = spent(b); const remain = Number(b.amount || 0) - used; const pct = Number(b.amount || 0) > 0 ? Math.min(100, used / Number(b.amount) * 100) : 0; if (editingId === b.id) return <Card key={b.id}><Title t="編輯預算" r={periodLabel(b)} /><div style={{ display: 'grid', gap: 10 }}><input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} style={input()} placeholder="預算名稱" /><select value={editForm.category} onChange={(e) => setEditForm({ ...editForm, category: e.target.value })} style={input()}>{budgetCategories.map((x) => <option key={x}>{x}</option>)}</select><input value={editForm.amount} onChange={(e) => setEditForm({ ...editForm, amount: e.target.value })} style={{ ...input(), fontSize: 20, fontWeight: 900 }} placeholder="預算金額" inputMode="decimal" /><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}><button onClick={() => saveEdit(b.id)} style={{ ...smallButton, color: '#bbf7d0', border: '1px solid rgba(34,197,94,.5)', background: 'rgba(34,197,94,.12)' }}>儲存</button><button onClick={cancelEdit} style={{ ...smallButton, color: '#cbd5e1', border: '1px solid rgba(148,163,184,.35)', background: 'transparent' }}>取消</button><button onClick={() => deleteBudget(b.id)} style={{ ...smallButton, color: '#fca5a5', border: '1px solid rgba(248,113,113,.35)', background: 'rgba(127,29,29,.18)' }}>刪除</button></div></div></Card>; return <Card key={b.id}><Title t={b.name} r={periodLabel(b)} /><Metric label={b.category} value={nt(used)} sub={`${remain < 0 ? '超支 ' + nt(Math.abs(remain)) : '剩餘 ' + nt(remain)}｜預算 ${nt(b.amount)}｜已用 ${pct.toFixed(0)}%`} bad={remain < 0} />{rows.length > 0 && <div style={{ marginTop: 10 }}><div style={{ color: '#bae6fd', fontSize: 12, fontWeight: 900 }}>支出明細</div><TxList rows={rows} budgets={budgets} /></div>}<button onClick={() => startEdit(b)} style={{ marginTop: 10, border: '1px solid rgba(212,175,55,.65)', borderRadius: 12, padding: 9, background: 'rgba(92,64,16,.45)', color: '#fff7bd', fontWeight: 900 }}>編輯</button></Card>; })}
  </>;
}
...
function AssetPage({ assets, setAssets }) {
  ...
  const totalTwd = manualTwd + hunterUsd * RATE;
  async function loadWalletLedger() { setLedger((x) => ({ ...x, loading: true, error: '' })); try { const res = await fetch('/api/wallet-ledger?t=' + Date.now(), { cache: 'no-store' }); const json = await res.json(); if (!res.ok || json.ok === false) throw new Error(json.message || json.error || 'wallet-ledger failed'); const marketValue = Number(json.portfolioMarketValue ?? json.currentValue ?? json.marketValue ?? 0); setLedger({ loading: false, error: '', marketValue, lastSyncTime: json.lastSyncTime || json.updatedAt || json.checkedAt || '', source: json.priceSource || json.source || '' }); } catch (error) { setLedger({ loading: false, error: error.message || '同步失敗', marketValue: 0, lastSyncTime: '', source: '' }); } }
  ...
  function startEdit(a) { setEditingId(a.id); setEditForm({ name: a.name, type: a.type, amount: String(a.amount ?? 0), note: a.note || '' }); }
  function cancelEdit() { setEditingId(''); setEditForm(emptyAsset); }
  function saveEdit(id) { const amount = Number(editForm.amount); if (!editForm.name.trim() || !Number.isFinite(amount)) return; setAssets(assets.map((a) => a.id === id ? { ...a, name: editForm.name.trim(), type: editForm.type, amount, note: editForm.note.trim() || '手動輸入' } : a)); cancelEdit(); }
  function deleteAsset(id) { setAssets(assets.filter((a) => a.id !== id)); if (editingId === id) cancelEdit(); }
  const smallButton = { borderRadius: 10, padding: '7px 10px', fontWeight: 900, background: 'transparent' };
  return <>
    <Card><Title t="綜合總資產" r="TWD 估算" /><div style={{ fontSize: 42, fontWeight: 1000 }}>{nt(totalTwd)}</div><p style={{ color: '#94a3b8', lineHeight: 1.55 }}>手動資產 {nt(manualTwd)} + 獵人投資 {usd(hunterUsd)} × 匯率 {RATE}</p><div style={{ color: ledger.error ? '#fca5a5' : '#86efac', fontSize: 12, fontWeight: 900 }}>資料源：/api/wallet-ledger｜{ledger.loading ? '同步中' : ledger.error ? ledger.error : '已同步'}</div></Card>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}><Metric label="手動資產" value={nt(manualTwd)} /><Metric label="獵人投資市值" value={usd(hunterUsd)} good={!ledger.error && hunterUsd > 0} /></div>
    <Card><Title t="新增手動資產" r="Local DB" /><div style={{ display: 'grid', gap: 10 }}><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={input()} placeholder="資產名稱" /><select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} style={input()}>{assetTypes.map((x) => <option key={x}>{x}</option>)}</select><input value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} style={input()} placeholder="金額" inputMode="decimal" /><input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} style={input()} placeholder="備註" /><Btn onClick={addAsset}>新增資產</Btn><button onClick={loadWalletLedger} style={{ border: '1px solid rgba(34,197,94,.5)', borderRadius: 14, padding: 10, background: 'rgba(34,197,94,.12)', color: '#bbf7d0', fontWeight: 1000 }}>重新同步獵人投資</button></div></Card>
    <Card><Title t="手動資產清單" r={`${assets.length} 筆`} />{assets.map((a) => editingId === a.id ? <div key={a.id} style={{ display: 'grid', gap: 9, padding: '12px 0', borderBottom: '1px solid rgba(148,163,184,.12)' }}><input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} style={input()} /><select value={editForm.type} onChange={(e) => setEditForm({ ...editForm, type: e.target.value })} style={input()}>{assetTypes.map((x) => <option key={x}>{x}</option>)}</select><input value={editForm.amount} onChange={(e) => setEditForm({ ...editForm, amount: e.target.value })} style={input()} inputMode="decimal" /><input value={editForm.note} onChange={(e) => setEditForm({ ...editForm, note: e.target.value })} style={input()} /><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}><button onClick={() => saveEdit(a.id)} style={{ ...smallButton, color: '#bbf7d0', border: '1px solid rgba(34,197,94,.5)' }}>儲存</button><button onClick={cancelEdit} style={{ ...smallButton, color: '#cbd5e1', border: '1px solid rgba(148,163,184,.35)' }}>取消</button><button onClick={() => deleteAsset(a.id)} style={{ ...smallButton, color: '#fca5a5', border: '1px solid rgba(248,113,113,.35)' }}>刪除</button></div></div> : <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 10, alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(148,163,184,.12)' }}><div><b>{a.name}</b><div style={{ color: '#94a3b8', fontSize: 12 }}>{a.type}｜{a.note}</div></div><b style={{ color: '#86efac' }}>{nt(a.amount)}</b><button onClick={() => startEdit(a)} style={{ ...smallButton, color: '#fff7bd', border: '1px solid rgba(212,175,55,.55)', background: 'rgba(92,64,16,.4)' }}>編輯</button></div>)}</Card>
  </>;
}
...
export default function FinancialOS() {
  ...
  useEffect(() => { try { const raw = localStorage.getItem(TX_KEY) || localStorage.getItem(OLD_TX_KEY); setTxs(raw ? cleanDb(JSON.parse(raw)) : sample); } catch { setTxs(sample); } try { const raw = JSON.parse(localStorage.getItem(BUDGET_KEY) || 'null'); if (Array.isArray(raw)) setBudgets(cleanBudgets(raw)); } catch {} try { const raw = JSON.parse(localStorage.getItem(ASSET_KEY) || 'null'); if (Array.isArray(raw)) setAssets(cleanAssets(raw)); } catch {} setReady(true); }, []);
