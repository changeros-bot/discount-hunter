...
function Dashboard({ txs }) {
  ...
  return <>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}><Metric label="區間收入" value={nt(s.income)} good={s.income > 0} /><Metric label="薪轉收入" value={nt(s.salaryTransfer)} good={s.salaryTransfer > 0} /><Metric label="區間支出" value={nt(s.expense)} /><Metric label="投資扣款" value={nt(s.invest)} /><Metric label="生活費（10日週期）" value={nt(livingStats.life)} sub={`${cycle.start}～${cycle.end}｜${remain < 0 ? `超支 ${nt(Math.abs(remain))}` : `剩餘 ${nt(remain)}`}`} bad={remain < 0} /></div>
    <Card><Title t="區間大類支出" r={`${s.groups.length} 類`} />{s.groups.map((g) => <div key={g.name} style={{ display: 'grid', gridTemplateColumns: '82px 1fr auto', gap: 10, padding: '7px 0' }}><b>{g.name}</b><div style={{ height: 12, borderRadius: 99, background: '#243044', overflow: 'hidden', marginTop: 4 }}><div style={{ height: '100%', width: Math.min(100, Math.max(1, g.amount / Math.max(1, s.expense) * 100)) + '%', background: 'linear-gradient(90deg,#38bdf8,#22c55e)' }} /></div><b>{nt(g.amount)}</b></div>)}</Card>
...
function Budget({ txs, budgets, setBudgets }) {
  ...
  function spent(b) {
    if (b.id === 'b-living' || b.mode === 'living_cycle') return livingStats.life;
    if (b.id === 'b-invest' || b.mode === 'investment') return monthStats.invest;
    return txs.filter((t) => t.type === '支出' && t.budgetId === b.id).reduce((sum, t) => sum + Number(t.amount || 0), 0);
  }
  ...
  function addBudget() {
    const amount = Number(form.amount || 0);
    if (!amount) return;
    setBudgets([{ id: 'b' + Date.now(), name: form.name.trim() || form.category, category: form.category, amount, mode: 'project' }, ...budgets]);
    setForm(emptyBudget);
  }
