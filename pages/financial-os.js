...
const STORAGE_KEY="josh-financial-os-transactions-20260705-v8";
...
const usd=n=>`USD ${num(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}`;
const sum=xs=>xs.reduce((s,x)=>s+num(x.amount),0);
const inRange=(t,r)=>String(t.date||"")>=r.start&&String(t.date||"")<=r.end;
const rangeLabel=r=>`${r.start.replaceAll("-","/")} - ${r.end.replaceAll("-","/")}`;
...
const seedTransactions=rawRows.split("\n").map((line,i)=>{const[date,note,amount]=line.split("|");const category=cat(note);return{id:`seed-${date}-${i}-${note}`,date,type:"支出",amount:num(amount),account:"自用",category,note,isLivingExpense:category==="投資"?"N":"Y",isFixedExpense:isFixed(category)?"Y":"N",affectsBudget:"Y"}});
function stats(transactions,range){const tx=transactions.filter(t=>inRange(t,range));const expenses=tx.filter(t=>t.type==="支出");const income=sum(tx.filter(t=>t.type==="收入"));const expense=sum(expenses);const living=sum(expenses.filter(t=>t.isLivingExpense==="Y"));const fixed=sum(expenses.filter(t=>t.isFixedExpense==="Y"||isFixed(t.category)));const groups=["飲食","服飾","居住","交通","教育","娛樂","醫療","金融","固定支出","生活用品","其他"].map(name=>({name,amount:sum(expenses.filter(t=>groupOf(t.category)===name))})).filter(x=>x.amount>0);const foods=FOOD_CATEGORIES.map(name=>({name,amount:sum(expenses.filter(t=>t.category===name))}));return{tx,expenses,income,expense,living,fixed,groups,foods}}
...
function Bar({amount,max,danger}){const raw=max>0?Math.round(amount/max*100):0;const width=amount>0?Math.min(100,Math.max(3,raw)):0;return <div style={{height:12,borderRadius:999,background:amount===0?"rgba(15,23,42,.82)":"#243044",border:amount===0?"1px dashed rgba(148,163,184,.20)":"none",overflow:"hidden"}}><div style={{height:"100%",width:`${width}%`,borderRadius:999,background:danger?"linear-gradient(90deg,#f59e0b,#ef4444)":"linear-gradient(90deg,#38bdf8,#22c55e)"}} /></div>}
