...
function aggregatePositions(rows = []) {
  ...
  for (const row of rows || []) {
    ...
    map.set(k, {
      ...
      rules: current.rules?.length ? current.rules : row.rules,
      ...
      dateKey: current.dateKey || row.dateKey,
    });
  }
...
export async function getServerSideProps({ req, query }) {
  ...
  try {
    const json = slimSummary(await readJson(`${base}/api/v17/paper-summary?ssr=1&t=${Date.now()}`));
...
export default function PaperAutoPage({ initialSummary = null, initialRun = null, initialError = "" }) {
  ...
  const rawRows = Array.isArray(summary.positions) ? summary.positions : [];
  const grouped = aggregatePositions(rawRows);
  ...
  const displayCoreCount = core.length || Number(apiGroups?.["既有V17十檔"]?.symbolCount ?? summary?.summary?.existingTenCount ?? 0);
  const displayPreparedCount = prepared.length || Number(apiGroups?.["預備名單"]?.symbolCount ?? summary?.summary?.preparedListCount ?? 0);
  ...
  const displayPnlPct = portfolio.cost > 0 ? portfolio.pnl / portfolio.cost : Number(summary?.summary?.pnlPct || 0);
  const displayTotal = grouped.length || Number(summary?.summary?.symbolCount || 0);
  const coverageTotal = displayTotal || 28;
  ...
  return <main style={{ minHeight: "100vh", color: "#f8fafc", background: "linear-gradient(180deg,#020617 0%,#07111f 55%,#0f172a 100%)", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans TC',Arial,sans-serif" }}><div style={{ maxWidth: 560, margin: "0 auto", padding: "22px 14px 40px" }}>
    ...
    <header style={{ marginTop: 18, marginBottom: 14 }}><div style={{ color: "#22c55e", letterSpacing: 3, fontWeight: 1000, fontSize: 13 }}>V17 紙上交易自動測試</div><h1 style={{ fontSize: 30, lineHeight: 1.05, margin: "10px 0", fontWeight: 1000 }}>紙上交易總控台</h1><p style={{ color: "#cbd5e1", lineHeight: 1.5, fontWeight: 850, margin: 0 }}>折扣獵人主頁只放正式上線 10 檔；預備名單只能在本頁驗證，升格必須 Josh 明確同意。</p></header>
    {initialError ? <Box title="錯誤" tone="red"><div style={{ color: "#fecaca", fontWeight: 850 }}>{initialError}</div></Box> : null}
    <Box title="總覽" tone="green"><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, color: "#cbd5e1", fontWeight: 850, fontSize: 13 }}><div>模式：{summary?.settings?.mode || "AUTO_PAPER"}</div><div>驗證期：4 週</div><div>核心正式：{displayCoreCount} 檔</div><div>預備名單：{displayPreparedCount} 檔</div><div>紙上批次：{rawLotCount} 筆</div><div>進度條：{progressCount}/{displayPreparedCount}</div><div>投入成本：${n(displayCost)}</div><div>目前市值：${n(displayValue)}</div><div>損益：<strong style={{ color: pnlColor }}>${n(displayPnl)}</strong></div><div>報酬率：<strong style={{ color: pnlColor }}>{n(displayPnlPct * 100)}%</strong></div><div>真實下單：禁止</div></div><div style={{ marginTop: 10, padding: 10, borderRadius: 14, background: "rgba(2,6,23,.38)", border: "1px solid rgba(148,163,184,.12)", color: "#cbd5e1", fontWeight: 850, fontSize: 12, lineHeight: 1.6 }}>紙上交易總名單 {displayTotal} 檔｜核心10檔 {displayCoreCount}｜預備名單 {displayPreparedCount}｜SSR slim payload｜資料源：/api/v17/paper-summary</div></Box>
    <Box title="操作"><LinkButton href={`/paper-auto?action=run&t=${now}`}>檢查今日紙上交易狀態</LinkButton><LinkButton href={`/paper-auto?refresh=${now}`} tone="blue">重新整理</LinkButton><div style={{ marginTop: 9, color: "#94a3b8", fontSize: 12, fontWeight: 850, lineHeight: 1.45 }}>控制已改成網址觸發；本版已壓縮 SSR 傳輸資料，不再把完整 paper-summary 塞進頁面。</div>{initialRun ? <div style={{ marginTop: 10, color: "#bbf7d0", fontWeight: 900, lineHeight: 1.55 }}><div>今日紙上交易檢查完成。</div><div>紙上交易總名單：{coverageTotal} 檔。</div><div>本按鈕掃描：{scanned} 檔。</div><div>已由獨立批次在線：{Math.max(0, coverageTotal - scanned)} 檔。</div><div>本次新增 {initialRun.createdCount} 筆，略過 {initialRun.skippedCount} 筆。</div></div> : null}{initialRun?.skipped?.length ? <details style={{ marginTop: 8, color: "#fde68a", fontSize: 12, fontWeight: 850, lineHeight: 1.45 }}><summary>查看略過明細</summary>{initialRun.skipped.map((x) => <div key={`${x.symbol}-${x.reason}`}>{x.symbol}：{x.reason}</div>)}</details> : null}</Box>
